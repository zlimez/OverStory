...
{
    /// <summary>
    /// 본의 IK 연산 순서를 결정하는 노드 래퍼 클래스
    /// 본이 Chain이 있거나 Single Look At이 있는 경우에 이 노드로 저장된다.
    /// 메시 그룹의 멤버로서 저장되며, IK 갱신시 같이 갱신된다.
    /// </summary>
    public class apBoneIKRunNode
    {
        // Members
        //----------------------------------
        public apBone _linkedBone = null;
        public apBone _linkedChainRootBone = null;//IK가 연산되는 Chain의 Root 본
        ...
        public apBoneIKRunNode(apBone bone, int index, apBone chainRootBone)
        {
            _linkedBone = bone;
