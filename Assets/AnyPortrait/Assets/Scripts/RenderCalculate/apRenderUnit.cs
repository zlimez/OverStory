...
namespace AnyPortrait
{
	...
	public class apRenderUnit
	{
				...
				if (_meshGroupTransform != null)
				{
					...
					if (_parentRenderUnit != null)
					{
						if (_parentRenderUnit.WorldMatrixWrap != null
							&& _parentRenderUnit.WorldMatrixWrapWithoutModified != null)
						{
							_meshGroupTransform.AddWorldMatrix_Parent(	_parentRenderUnit.WorldMatrixWrap,
																		_parentRenderUnit.WorldMatrixWrapWithoutModified);
		...
		//v1.4.4 추가
		/// <summary>
		/// 모디파이어 값의 기본이 되는 모디파이어가 적용되지 않은 World Matrix를 계산한다.
		/// 업데이트용 World Matrix는 여기서 계산하지 않는다. (모디파이어 Calculate 처리 전일 수 있으므로)
		/// </summary>
		private void MakeNoModWorldMatrix()
		{
			//1. 부모 데이터 넣기
			apMatrix parentNoModWorldMatrix = null;
			if(_parentRenderUnit != null)
			{
				parentNoModWorldMatrix = _parentRenderUnit.WorldMatrixWrapWithoutModified;
			}

			//Debug.LogError("MakeNoModWorldMatrix Render Unit");

			if(_meshGroupTransform != null)
			{
				_meshGroupTransform.MakeNoModWorldMatrix(parentNoModWorldMatrix);
			}
			else if (_meshTransform != null)
			{
				_meshTransform.MakeNoModWorldMatrix(parentNoModWorldMatrix);
			}
		}
		...
		/// 이 함수가 호출되어야 할 때
		/// > 메시 그룹 선택시 (Root Unit, Animation 포함)
		/// > 메시 그룹 기본 Transform 편집 종료시
		/// > 애니메이션에서 메시 그룹 선택시
		/// </summary>
		public enum CALCULATE_NO_MOD_POS_REQUEST
		{
			/// <summary>Matrix 갱신 + 단일</summary>
			RefreshWorldMatrix_Single,
			/// <summary>Matrix 갱신 + 자식도 계산</summary>
			RefreshWorldMatrix_WithChild,
			/// <summary>계산된 Matrix를 그냥 사용 + 단일</summary>
			OnlyAdaptMatrix_Single,
			/// <summary>계산된 Matrix를 그냥 사용 + 자식도 계산</summary>
			OnlyAdaptMatrix_WithChild
		}
		public void CalculateRenverVertsWorldPosNoMod(CALCULATE_NO_MOD_POS_REQUEST requestType)
		{
			//World Matrix (No-Mod) 갱신 옵션
			if(requestType == CALCULATE_NO_MOD_POS_REQUEST.RefreshWorldMatrix_Single
				|| requestType == CALCULATE_NO_MOD_POS_REQUEST.RefreshWorldMatrix_WithChild)
			{
				//Update_Pre(0.0f);//World Matrix 전체 계산
				MakeNoModWorldMatrix();//No-Mod World Matrix만 계산
			}
			...
			if (_unitType == UNIT_TYPE.Mesh)
			{
				...
				//변경 v1.4.4 : 행렬 계산을 RenderVert마다 하는 코드에서 Mesh 단위로 한다.
				//apMatrix3x3 matrix_ToWorld = _meshTransform._mesh.Matrix_VertToLocal * WorldMatrixWrapWithoutModified.MtrxToSpace;//[버그. 수식 틀림]
				apMatrix3x3 matrix_ToWorld = WorldMatrixWrapWithoutModified.MtrxToSpace * _meshTransform._mesh.Matrix_VertToLocal;//[v1.4.5] 수식 수정함. 이게 맞음

				//디버그
				//Debug.LogError("Calculate Render Vert Matrix >>>");
				//Debug.Log(" - Mesh Vert To Local : \n" + _meshTransform._mesh.Matrix_VertToLocal.ToString());
				//Debug.Log(" - World Matrix No Mod : \n" + WorldMatrixWrapWithoutModified.MtrxToSpace.ToString());
				//Debug.Log(" - World Matrix Result : \n" + matrix_ToWorld.ToString());
				...
				if (nRenderVerts > 0)
				{
					for (int i = 0; i < nRenderVerts; i++)
					{
						...
						//변경 v1.4.4 : 계산을 외부로 빼서 성능 향상
						rVert.CalculateNotModified(ref matrix_ToWorld);
					}
				}
			}

			if(requestType == CALCULATE_NO_MOD_POS_REQUEST.OnlyAdaptMatrix_Single
				|| requestType == CALCULATE_NO_MOD_POS_REQUEST.RefreshWorldMatrix_Single)
			{
				//자식은 처리하지 않는다.
				return;
			}

			int nChildRenderUnits = _childRenderUnits != null ? _childRenderUnits.Count : 0;
			if(nChildRenderUnits == 0)
			{
				return;
			}

			for (int i = 0; i < nChildRenderUnits; i++)
			{
				_childRenderUnits[i].CalculateRenverVertsWorldPosNoMod(requestType);
			}
		}
