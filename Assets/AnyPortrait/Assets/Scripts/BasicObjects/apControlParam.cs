...
namespace AnyPortrait
{
	...
	public class apControlParam
	{
		...
		//v1.4.5 : 보간이 안되는 인덱스 타입 (Int형 한정)
		public bool _isNonInterpolatedIndex = false;
		...
		public class LayeredRequest
		{
			...
			private int _cal_IndexOfIntParam = 0;
			...
			public void CompleteCalculate()
			{
				...
				if(_isNeedToSort)
				{
					...
					//변경 v1.4.5 : 대리자 이용 (메모리 할당없어짐)
					_params.Sort(Compare_ControlParamResult);
				}
			...
			private static int Compare_ControlParamResult(apAnimControlParamResult a, apAnimControlParamResult b)
			{
				return a._animPlayOrder - b._animPlayOrder;
			}
			...
			/// <summary>
			/// 현재 애니메이션 레이어에서 Index의 값을 리턴한다.
			/// </summary>
			/// <returns></returns>
			public int GetCalculated_IntIndex()
			{
				//같은 레이어에서 애니메이션의 전환 과정에서 
				_cal_CurParam = _params[0];

				_cal_IndexOfIntParam = _cal_CurParam.GetIndexValueOfIntParam();//해당 애니메이션 내의 키프레임 간의 값을 연산한 결과
				...
				if (_nParams > 1)
				{
					for (int i = 1; i < _nParams; i++)
					{
						...
						//중요
						//Anim Weight가 0.5가 넘어가면 애니메이션이 전환된 것으로 간주한다.
						if(_cal_CurParam._animWeight > 0.5f)
						{
							_cal_IndexOfIntParam = _cal_CurParam.GetIndexValueOfIntParam();
						}
					}
				}

				return _cal_IndexOfIntParam;
			}
		...
		[NonSerialized] private float _cal_Value_1D = 0.0f;
		[NonSerialized] private Vector2 _cal_Value_2D = Vector2.zero;

		//추가 v1.4.5 : Index 타입의 Int 컨트롤 파라미터라면 다른 값을 가진다.
		[NonSerialized] private int _cal_IndexOfIntParam = -1;
		...
		{
			int nLayerRequests = _layeredRequests != null ? _layeredRequests.Count : 0;

			if (!_isUpdated || nLayerRequests == 0)
			{
							...
							case TYPE.Int:
								{
									...
									_cal_IndexOfIntParam = _lastValue_Int;
			...
			for (int iLayer = 0; iLayer < nLayerRequests; iLayer++)
			{
				...
				//추가 v1.4.5
				//인덱스 타입은 보간이 되지 않으므로, Weight가 0.5를 넘기면 바로 전환한다.
					...
					|| _curLayeredRequest._blendMethod == apAnimPlayUnit.BLEND_METHOD.Interpolation)
				{
					switch (_valueType)
					{
						case TYPE.Int:
							{
								if(_isNonInterpolatedIndex)
								{
									//인덱스 방식 (v1.4.5)
									//Weight가 0.5를 넘기면 바로 전환하자
									//- 레이어 순서(애니메이션 호출 순서)에서 가중치가 0.5를 넘기면 전환
									//- 또는 첫번째 레이어(첫번째 애니메이션)면 바로 할당

									if(iCalculatedLayer == 0 || _curLayeredRequest._totalWeight > 0.5f)
									{
										//키프레임에 의한 인덱스를 할당
										_cal_IndexOfIntParam = _curLayeredRequest.GetCalculated_IntIndex();
									}
								}
								else
								{
				...
				case TYPE.Int:
					{
						...
						{
							//인덱스 방식 (v1.4.5)
							//아예 보간이 없으므로, 계산된 값을 Clamped만 한 후 바로 할당한다.
							_int_ClampedCalculatedLayer = Mathf.Clamp(_cal_IndexOfIntParam, _int_Min, _int_Max);
							_int_Cur = _int_ClampedCalculatedLayer;
						}
						else
						{
							//일반 보간식
		...
		public void CopyFromControlParam(apControlParam srcParam)
		{
			...
			//v1.4.5 : 보간이 안되는 인덱스 타입 (Int형 한정)
			_isNonInterpolatedIndex = srcParam._isNonInterpolatedIndex;
		}
