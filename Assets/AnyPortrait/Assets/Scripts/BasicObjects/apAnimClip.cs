...
namespace AnyPortrait
{
	...
	public class apAnimClip
	{
		...
		{
			int nParamResults = _controlParamResult.Count;
			if (nParamResults == 0)
			{
				return;
			}

			for (int i = 0; i < nParamResults; i++)
			{
			...
			bool isKeyChecked_A = false;
			bool isKeyChecked_B = false;


			if (nTimelines > 0)
			{
				for (int iTL = 0; iTL < nTimelines; iTL++)
				{
					...
					for (int iL = 0; iL < nLayers; iL++)
					{
						...
						int nKeyframes = layer._keyframes != null ? layer._keyframes.Count : 0;
						...
						//UnityEngine.Debug.Log("키프레임 - " + nKeyframes + "개");

						////디버그용
						//for (int iK = 0; iK < nKeyframes; iK++)
						//{
						//	curKeyframe = layer._keyframes[iK];
						//	prevKeyframe = curKeyframe._prevLinkedKeyframe;
						//	nextKeyframe = curKeyframe._nextLinkedKeyframe;

						//	UnityEngine.Debug.Log("- 키 : " + curKeyframe._frameIndex 
						//		+ "[ Active : " + curKeyframe._activeFrameIndexMin + " ~ " + curKeyframe._activeFrameIndexMax + "] "
						//		+ "[ Loop Start : " + curKeyframe._isLoopAsStart + " / Loop End : " + curKeyframe._isLoopAsEnd + " ] "
						//		+ " [ Prev : " + (prevKeyframe != null ? prevKeyframe._frameIndex.ToString() : "X ")
						//		+ " / Next : " + (nextKeyframe != null ? nextKeyframe._frameIndex.ToString() : "X") + " ]");
						//}


						//v1.4.6
						//루프의 마지막 키프레임에서 보간 에러가 발생하는 문제
						//<이유>
						//- 현재 재생되는 프레임이 키프레임에 위치할때, 보간은 2개가 아닌 3개의 경우가 발생한다.
						//- (1) [Prev] > Cur, (2) [Cur], (3) Cur > [Next]
						//- 근데, 실제로는 프레임에 따라서 (2)만 유효하고 (1), (3) (Prev, Next 기준)은 무시되어야 한다.
						//- 일반적으로 for문 처리 순서에 의해서 (3)이 처리 되기 전에 (1), (2)가 계산되므로, (2)가 계산에 포함되어 성공
						//- 근데 "Loop + 마지막 키프레임"인 경우엔 (3) 연산이 맨 첫번째 키프레임이 이므로, 계산 순서가 (3) > (1) > (2)가 된다.
						//- 결과적으로 (2)가 계산되기 전에 (3)과 (1)이 완료되어 A, B가 완료된 것으로 인식된다.
						//- 즉 "Loop + 마지막 키프레임"에서만 보간 연산이 잘못됨

						//<해결 방법>
						//- 보간시, Next, Prev의 키프레임에 현재 키프레임이 있다면 (Cur를 제외하곤 Dummy는 고려하지 않음)
						//- 이건 Next, Prev에 프레임이 있다는 것이므로 보간에 포함시키지 않는다.


						//찾아야 되는 키프레임은 2개 또는 1개다.
						//A ~(보간)~ B이므로, A와 B를 다 찾았다면 더 찾을 필요는 없다.

						isKeyChecked_A = false;
						isKeyChecked_B = false;
						...
						for (int iK = 0; iK < nKeyframes; iK++)
						{
							...
							if (curFrame == curKeyframe._frameIndex ||
								(
									(curKeyframe._isLoopAsStart || curKeyframe._isLoopAsEnd) 
									&& curKeyframe._loopFrameIndex == curFrame)
								)
							{
								//보간이 필요없으므로 100%로 할당
								cpResult.SetKeyframeResult_SingleKey(curKeyframe);
							...
							else if (curKeyframe.IsFrameIn(curFrame, apAnimKeyframe.LINKED_KEY.Prev))
							{
								...
								if (prevKeyframe != null)
								{
									...
									//v1.4.6 : 재생되는 프레임이 Prev 키프레임에 딱맞는 상태라면 보간을 생략한다. [위의 "Loop + 마지막 키프레임" 버그 해결]
									...
									//Prev > Cur이므로, Cur은 Key B(Next)이다.
									cpResult.SetKeyframeResult(curKeyframe, itp, apAnimControlParamResult.RESULT_KEY.KeyB);
							...
							else if (curKeyframe.IsFrameIn(curFrame, apAnimKeyframe.LINKED_KEY.Next))
							{
								...
								if (nextKeyframe != null)
								{
									...
									//v1.4.6 : 재생되는 프레임이 Next 키프레임에 딱맞는 상태라면 보간을 생략한다. [위의 "Loop + 마지막 키프레임" 버그 해결]
									...
									//Cur > Next이므로, Cur은 Key A(Prev)이다.
									cpResult.SetKeyframeResult(curKeyframe, itp, apAnimControlParamResult.RESULT_KEY.KeyA);
							...
							if(isKeyChecked_A && isKeyChecked_B)
							{
								//이미 A, B 키프레임을 모두 찾았으므로 더 찾을 필요가 없다.
								break;
							}
						}
					}
		...
		public void UpdateControlParam_Opt(int optLayer = 0, float optWeight = 1.0f, apAnimPlayUnit.BLEND_METHOD optBlendMethod = apAnimPlayUnit.BLEND_METHOD.Interpolation)
		{
			...
			for (int iTL = 0; iTL < _nOptControlParamTimelineLayers; iTL++)
			{
				...
				if (targetLUT._keyframe_Cur == targetLUT._keyframe_Next)
				{
					...
					cpResult.SetKeyframeResult_SingleKey(targetLUT._keyframe_Cur);
				}
				else
				{
					//두개의 영역에 들어왔다.
					...
					//Runtime에선 Float Frame
					cpResult.SetKeyframeResult(keyframe_A, keyframe_A._curveKey.GetItp_Float(frameFloat_ForA, false, frameInt_ForA), apAnimControlParamResult.RESULT_KEY.KeyA);

					//Key B 계산
					//cpResult.SetKeyframeResult(keyframe_B, keyframe_B._curveKey.GetItp_Int(frameInt_ForB, true));//Editor에선 Int Frame

					//Runtime에선 Float Frame
					cpResult.SetKeyframeResult(keyframe_B, keyframe_B._curveKey.GetItp_Float(frameFloat_ForB, true, frameInt_ForB), apAnimControlParamResult.RESULT_KEY.KeyB);
				}
			}
