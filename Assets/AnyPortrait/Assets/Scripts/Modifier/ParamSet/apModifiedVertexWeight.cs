...
namespace AnyPortrait
{
	...
	public class apModifiedVertexWeight
	{
		...
		//삭제 v1.4.4 : 상황에 따라 Render Vertex의 값과 동기화과 되지 않는 문제
		//더이상 별도의 값으로 가지지 않고 그냥 Render Vertex의 값을 사용하도록 하자
		//[SerializeField]
		...
		public void InitCalculatedValue()
		{
			...
			if (_isPhysics)
			{
				...
				//삭제 v1.4.4 : 항상 Render Vertex로 부터 받는다.
		...
		public void LinkModMeshAndWeights(apModifiedMesh modifiedMesh)
		{
			...
			if (_isPhysics)
			{
				...
				if (_renderVertex != null)
				{
					//삭제 v1.4.4 : 값을 저장하는게 아니라 항상 Render Vertex를 참조하도록
					//_pos_World_NoMod = _renderVertex._pos_World_NoMod;
		...
		public void RefreshModMeshAndWeights_Check(apModifiedMesh modifiedMesh)
		{
			...
			if (_isPhysics)
			{
				...
				if (_renderVertex != null)
				{
					//삭제 v1.4.4 : 항상 Render Vertex를 참조한다.
					//_pos_World_NoMod = _renderVertex._pos_World_NoMod;
		...
		public void RefreshLinkedVertex()
		{
			if (_isPhysics)
			{
				...
				{
					//v1.4.4 추가
					//연결된 버텍스가 없다면 Refresh가 불가능하다.
					//RenderUnit 생성보다 빠르게 Link를 시도할 경우 이 문제가 발생하는데,
					//Render Unit 생성 후 Link를 한번 더 호출하면 해결된다.
					//Debug.LogError("에러 : RefreshLinkedVertex : Render Vertex가 연결되지 않았다.");// > 에러 로그는 무시하고 return만 하면 된다.
					return;
				}
				//else
				//{
				//	Debug.Log("RefreshLinkedVertex 실행됨 [" + _renderVertex._pos_World_NoMod + "]");
				//}

				int nLinkVerts = _physicParam._linkedVertices != null ? _physicParam._linkedVertices.Count : 0;

				

				Vector2 pos_World_NoMod = _renderVertex._pos_World_NoMod;

				if (nLinkVerts > 0)
				{
					for (int i = 0; i < nLinkVerts; i++)
					{
						...
						//변경 v1.4.4 : _pos_World_NoMod 값을 RenderVert로 부터 받아온다.
						if(linkVert._modVertWeight._renderVertex == null)
						{ 
							continue;
						}
						float distW = (linkVert._modVertWeight._renderVertex._pos_World_NoMod - pos_World_NoMod).magnitude;
