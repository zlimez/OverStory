...
namespace AnyPortrait
{
	...
	public class apModifierBase : MonoBehaviour
	{
		...
		private static Vector2 _cal_Rig_TmpPos1_Vert2Local = Vector2.zero;
		private static Vector2 _cal_Rig_TmpPos2_Vert2Local = Vector2.zero;

		private static apMatrix _cal_tmpRig_matx_MeshW_NoMod = null; 
		private static apMatrix3x3 _cal_tmpRig_matx_MeshW_NoMod_Matrix3x3_Inv = apMatrix3x3.identity;
		private static apMatrix3x3 _cal_tmpRig_matx_MeshW_NoMod_Matrix3x3 = apMatrix3x3.identity;
		...
		private static apMatrix3x3 _cal_Rig_Matx_boneWorld_Mod_Matrix3x3 = apMatrix3x3.identity;
		private static apBoneWorldMatrix _cal_Rig_Matx_boneWorld_Default = null;
		private static apMatrix3x3 _cal_Rig_Matx_boneWorld_Default_Matrix3x3_Inv = apMatrix3x3.identity;
								...
								for (int iVert = 0; iVert < _cal_NumVerts; iVert++)
								{
									...
									for (int iWeightPair = 0; iWeightPair < nPairs; iWeightPair++)
									{
										...
										else
										{
											...
											//변경 v1.4.4 : * 연산자 덜 사용하기
											curveVert2WorldMatrix.SetAndMultiply(ref curveCurveMatrix, ref curPinWeightPair._curveDefaultMatrix_Inv);
			...
			for (int iCalParam = 0; iCalParam < _calculatedResultParams.Count; iCalParam++)
			{
				...
				for (int iSubList = 0; iSubList < _cal_SubParamGroupList.Count; iSubList++)
				{
					...
					for (int iPV = 0; iPV < _cal_NumParamKeys; iPV++)
					{
						...
						//변경 v1.4.4 : 아래 수식에서 불필요한 연속 참조를 막기 위해 변수에 저장
						_cal_tmpRig_matx_MeshW_NoMod_Matrix3x3_Inv = _cal_tmpRig_matx_MeshW_NoMod.MtrxToLowerSpace;
						_cal_tmpRig_matx_MeshW_NoMod_Matrix3x3 = _cal_tmpRig_matx_MeshW_NoMod.MtrxToSpace;
						...
						for (int iPos = 0; iPos < _cal_NumVerts; iPos++)
						{
							...
							//_cal_Rig_VertPosW_NoMod 값은 사실 업데이트 중에는 바뀌지 않는 고정값이다.
							//에디터라서 매번 계산해야함
							//이전
							...
							//변경 v1.4.4 : Ref를 이용
							apMatrix3x3.MultiplyPoint(ref _cal_Rig_TmpPos1_Vert2Local, ref _cal_Rig_matx_Vert2Local, ref _cal_Rig_curVertRig._vertex._pos);
							_cal_tmpRig_matx_MeshW_NoMod.MulPoint2(ref _cal_Rig_VertPosW_NoMod, ref _cal_Rig_TmpPos1_Vert2Local);
							...
							for (int iWeight = 0; iWeight < _cal_Rig_curVertRig._weightPairs.Count; iWeight++)
							{
								...
								//추가 v1.4.4 : 빠른 행렬 계산을 위해 아래의 식에 사용될 값을 여기서 저장
								_cal_Rig_Matx_boneWorld_Mod_Matrix3x3 = _cal_Rig_Matx_boneWorld_Mod.MtrxToSpace;
								

								_cal_Rig_Matx_boneWorld_Default = _cal_Rig_WeightPair._bone._worldMatrix_NonModified;
								_cal_Rig_Matx_boneWorld_Default_Matrix3x3_Inv = _cal_Rig_Matx_boneWorld_Default.MtrxToLowerSpace;

								//World -> Bone Local
								//_cal_Rig_VertPos_BoneLocal = _cal_Rig_Matx_boneWorld_Default.InvMulPoint2(_cal_Rig_VertPosW_NoMod);//이전
								_cal_Rig_Matx_boneWorld_Default.InvMulPoint2(ref _cal_Rig_VertPos_BoneLocal, ref _cal_Rig_VertPosW_NoMod);//변경 v1.4.4 : Ref 이용

								//Bone Local -> World
								//_cal_Rig_VertPosW_BoneWorld = _cal_Rig_Matx_boneWorld_Mod.MulPoint2(_cal_Rig_VertPos_BoneLocal);//이전
								_cal_Rig_Matx_boneWorld_Mod.MulPoint2(ref _cal_Rig_VertPosW_BoneWorld, ref _cal_Rig_VertPos_BoneLocal);//변경 v1.4.4 : Ref 이용
								...
								//변경 v1.4.4 : Ref 이용
								_cal_tmpRig_matx_MeshW_NoMod.InvMulPoint2(ref _cal_Rig_TmpPos2_Vert2Local, ref _cal_Rig_VertPosW_BoneWorld);
								apMatrix3x3.MultiplyPoint(ref _cal_Rig_VertPosL_Result, ref _cal_Rig_matx_Vert2Local_Inv, ref _cal_Rig_TmpPos2_Vert2Local);
								...
								//변경 v1.4.4 : 속도가 느린 * 연산자 대신 ref 함수를 사용하자
								//위 식을 ref 형식으로 옮긴 것이다.
								_cal_Rig_Matx_Result.SetAndMultiply4(	ref _cal_tmpRig_matx_MeshW_NoMod_Matrix3x3_Inv, 
																		ref _cal_Rig_Matx_boneWorld_Mod_Matrix3x3,
																		ref _cal_Rig_Matx_boneWorld_Default_Matrix3x3_Inv,
																		ref _cal_tmpRig_matx_MeshW_NoMod_Matrix3x3);
					...
					for (int iPV = 0; iPV < _cal_NumParamKeys; iPV++)
					{
							...
							)
						{
							...
							for (int iPos = 0; iPos < _cal_NumVerts; iPos++)
							{
								...
								//위치 변경됨 [v1.4.4]
								//이전
								//_cal_Phy_srcVertPos_NoMod = _cal_Phy_modVertWeight._pos_World_NoMod;//v1.4.4 이거 위치 밖(위)으로 이동시킬것

								//변경 v1.4.4 : 저장된 값이 아닌 Render Vertex의 값을 참조한다. (동기화되지 않아서 발생하는 버그 방지)
								_cal_Phy_srcVertPos_NoMod = _cal_Phy_modVertWeight._renderVertex._pos_World_NoMod;


								//변경 v1.4.4 : 위치 이동 (안에서 위로)
								...
								if (isValidFrame)
								{
									...
									//디버그
									//if (_cal_Phy_modVertWeight._weight > 0.0f)
									//{
									//	//Debug.LogWarning("기본 위치 No-Mod : " + _cal_Phy_modVertWeight._pos_World_NoMod);
									//	Debug.LogWarning("기본 위치 No-Mod (Render Vert) : " + _cal_Phy_modVertWeight._renderVertex._pos_World_NoMod);
									//	Debug.LogWarning("기본 위치 Cur : " + _cal_Phy_modVertWeight._pos_Real);

									//	Debug.Log("No Mod 행렬 : \n" + _cal_Phy_modVertWeight._renderUnit.WorldMatrixWrapWithoutModified.MtrxToSpace.ToString());
									//}
									...
									for (int iLinkVert = 0; iLinkVert < _cal_Phy_physicVertParam._linkedVertices.Count; iLinkVert++)
									{
										...
										//변경 v1.4.4 : Render Vertex의 값을 항상 참조하는 것으로 변경
										_cal_Phy_linkVertPos_NoMod = tmpPhysics_linkedVert._modVertWeight._renderVertex._pos_World_NoMod;

										tmpPhysics_linkedVert._deltaPosToTarget_NoMod = _cal_Phy_srcVertPos_NoMod - _cal_Phy_linkVertPos_NoMod;

										//_cal_Phy_srcVertPos_Cur = _cal_Phy_modVertWeight._pos_Real;//이것도 위에서 할당
										...
										//---디버그-----------
										//Vector2 linkedRenderVertRealPos_NoMod = tmpPhysics_linkedVert._modVertWeight._renderVertex._pos_World_NoMod;
										//if (_cal_Phy_modVertWeight._weight > 0.0f)
										//{
										//	Debug.Log("- [" + iLinkVert + "] 연결된 Vert 위치 No-Mod : " + _cal_Phy_linkVertPos_NoMod);
										//	Debug.Log("- [" + iLinkVert + "] 연결된 Vert 위치 No-Mod (Render Vertex) : " + linkedRenderVertRealPos_NoMod);
										//	Debug.Log("- [" + iLinkVert + "] 연결된 Vert 위치 Cur : " + _cal_Phy_linkVertPos_Cur);
										//	Debug.Log("- [" + iLinkVert + "] 장력 차이값 No-Mod : " + _cal_Phy_deltaVec_0);
										//	Debug.Log("- [" + iLinkVert + "] 장력 차이값 Cur : " + _cal_Phy_deltaVec_Cur);
										//}
							...
							for (int iPos = 0; iPos < _cal_NumVerts; iPos++)
							{
								...
								//if(_cal_Phy_modVertWeight._weight > 0.0f)
								//{
								//	Vector2 debug_ResultPos = _cal_TmpPosList[iPos];
								//	Vector2 debug_SrcPosWorldNoMod = _cal_ParamKeyValue._modifiedMesh._vertWeights[iPos]._renderVertex._pos_World_NoMod;
								//	Debug.LogWarning("디버그 : " + debug_SrcPosWorldNoMod + " > " + debug_ResultPos);
								//}
							}
							//---------------------------- Pos List
						}
							...
							for (int iPos = 0; iPos < _cal_NumVerts; iPos++)
							{
								...
								if (isValidFrame)
								{
									...
									for (int iLinkVert = 0; iLinkVert < _cal_Phy_physicVertParam._linkedVertices.Count; iLinkVert++)
									{
										...
										//변경 v1.4.4 : Render Vertex의 값을 항상 참조
										_cal_Phy_linkVertPos_NoMod = tmpPhysics_linkedVert._modVertWeight._renderVertex._pos_World_NoMod;
										...
										//_cal_Phy_srcVertPos_Cur = _cal_Phy_modVertWeight._pos_Real;//삭제. 위치 위로 이동됨
										_cal_Phy_linkVertPos_Cur = tmpPhysics_linkedVert._modVertWeight._pos_Real;
