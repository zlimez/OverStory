...
namespace AnyPortrait
{
	...
	public class apAnimPlayDeferredRequest
	{
		...
		public class CommandUnit
		{
			...
			public COMMAND_TYPE _commandType = COMMAND_TYPE.None;
			...
			public CommandUnit(int index)
			{
				_index = index;
				_isValid = false;
				_commandType = COMMAND_TYPE.None;
			}
		...
		public void Ready()
		{
			//커서를 사용 전으로 돌린다.
			//_iCommand = 0;//이전
			_iCommand = -1;//[v1.4.5] 입력이 없다면 음수가 되도록 변경 (더 안전하게)
		}

		private CommandUnit GetNextCommand()
		{
			//v1.4.5
			//만약 첫번째 호출이라면 인덱스가 -1일 것이다.
			if(_iCommand < 0)
			{
				_iCommand = 0;//유효한 인덱스로 변경
			}
		...
		private void Validate()
		{
			...
			for (int i = _iCommand - 1; i >= 0; i--)
			{
				_curCmd = _commands[i];
				if (!_curCmd._isValid 
					|| _curCmd._commandType == COMMAND_TYPE.None)//이 부분 추가 [v1.4.5]
				{
					//비활성화되었다.
					continue;
				}
					...
					case COMMAND_TYPE.CrossFadeAt:
						{
							...
							//에러 검출 v1.4.5
							if(_curCmd._playData == null)
							{
								//알 수 없는 에러가 발생했다.
								Debug.LogError("[AnyPortrait] An invalid animation request was entered for an unknown reason.\n" +
											"Playback method [" + _curCmd._commandType + "]. If this issue occurs continuously, please contact team RainyRizzle.");
								break;
							}
		...
		private void ExecuteCommands()
		{
			...
			for (int i = 0; i < _iCommand; i++)
			{
				_curCmd = _commands[i];
				if (!_curCmd._isValid
					|| _curCmd._commandType == COMMAND_TYPE.None//v1.4.5
					)
				{
					//비활성화되었다.
					continue;
