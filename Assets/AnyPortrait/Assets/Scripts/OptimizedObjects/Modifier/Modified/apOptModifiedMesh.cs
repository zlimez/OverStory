...
namespace AnyPortrait
{
	...
	public class apOptModifiedMesh
	{
		...
		public bool Bake(apModifiedMesh srcModMesh, apPortrait portrait)
		{
			...
			{	
				Bake_VertexWeights(rootTransform, targetTransform, targetMesh, srcModMesh._vertWeights, srcModMesh);
			}
			else
			{
		...
		public void Bake_VertexWeights(apOptTransform rootTransform, apOptTransform targetTransform,
										apOptMesh targetMesh, List<apModifiedVertexWeight> modVertWeights, apModifiedMesh srcModMesh)
		{
			_rootTransform = rootTransform;
			_targetTransform = targetTransform;
			_targetMesh = targetMesh;
			...
			for (int i = 0; i < _nVertWeights; i++)
			{
				...
				optModVertWeight.Bake(srcModVertWeight, ref matrix_ToWorld_NoMod);
