...
namespace AnyPortrait
{
	...
	public class apOptModifierUnitBase
	{
		...
		//[v1.4.5] 삭제. 에디터와 달리 모든 모디파이어는 항상 동작한다.
		//[NonSerialized]
		...
		public void Link(apPortrait portrait, apOptTransform parentOptTransform)
		{
			...
			int nPSGs = _paramSetGroupList != null ? _paramSetGroupList.Count : 0;
		...
		public void Bake(apModifierBase srcModifier, apPortrait portrait, bool isUseModMeshSet)
		{
			...
			if(_paramSetGroupList == null)
			{
				_paramSetGroupList = new List<apOptParamSetGroup>();
			}
			_paramSetGroupList.Clear();

			int nSrcPSGs = srcModifier._paramSetGroup_controller != null ? srcModifier._paramSetGroup_controller.Count : 0;

			if (nSrcPSGs > 0)
			{
				for (int i = 0; i < nSrcPSGs; i++)
				{
		...
		{
			int nCalResultParam = _calculatedResultParams != null ? _calculatedResultParams.Count : 0;

			//계산이 불가능한 상황일 때, 계산 값만 초기화한다.
			if (nCalResultParam == 0)
			{
				return;
			}
			...
			for (int i = 0; i < nCalResultParam; i++)
			{
...
//#endif
			...
			for (int i = 0; i < _calculatedResultParams.Count; i++)
			{
				...
				for (int iSubList = 0; iSubList < subParamGroupList.Count; iSubList++)
				{
					...
					//변경 v1.4.7 : _isCalculated를 삭제하고 최적화된 코드 이용
					int nParamKeys = curSubList.NumCalculatedParamKeyValues;
					subParamKeyValueList = curSubList.CalculatedParamKeyValues;
					...
					for (int iPV = 0; iPV < nParamKeys; iPV++)
					{
						...
						//>> 최적화 코드)
						//이전
						//vertRequest._modWeightPairs[iPV].SetWeight(paramKeyValue._weight);//<<TODO : 이 부분이 툴과 달라서 "계산된 것만 적용하기"가 안되네..
						//변경 v1.4.7 : VertexRequest에도 _isCalculated를 삭제하는 전략을 적용했다.
						vertRequest.OnModWeightPairCalculated(paramKeyValue._linkedVertexRequestMWP, paramKeyValue._weight);
...
//#endif
				...
				//calParam.Calculate();//Static은 Calculate가 필요 없다 (v1.4.7)
				//-------------------------------------------------------
//#if UNITY_EDITOR
//				Profiler.EndSample();
//#endif
...
//#endif
		...
		private void Calculate_Physics(float tDelta)
		{
			...
			for (int iCalParam = 0; iCalParam < _calculatedResultParams.Count; iCalParam++)
			{
				...
				//-------------------------------------------------------
				//1. Param Weight Calculate
				//calParam.Calculate();//Static에선 필요없다 (v1.4.7)
				//-------------------------------------------------------
...
//#endif
			...
			int nCRP = _calculatedResultParams != null ? _calculatedResultParams.Count : 0;
			if(nCRP == 0)
			{
				return;
			}
			...
			for (int i = 0; i < nCRP; i++)
			{
				...
				//신버전
				int nCalParamVertRequests = calParamVertRequestList != null ? calParamVertRequestList.Count : 0;
				if (nCalParamVertRequests > 0)
				{
					for (int iVR = 0; iVR < nCalParamVertRequests; iVR++)
					{
						calParamVertRequestList[iVR].InitCalculate();
					}
				}
				...
				{
					//calParam._result_Color = new Color(0.5f, 0.5f, 0.5f, 1.0f);
					calParam._result_Color = _defaultColor;
					calParam._result_IsVisible = false;//Alpha와 달리 Visible 값은 false -> OR 연산으로 작동한다.
				}
				else
				{
				...
				//SubList (ParamSetGroup을 키값으로 레이어화된 데이터)를 순회하면서 먼저 계산한다.
				//레이어간 병합 과정에 신경 쓸것
				int nSubParamGroups = subParamGroupList != null ? subParamGroupList.Count : 0;
...
//#endif
			...
			for (int i = 0; i < nCRP; i++)
			{
				...
				int nAnimClips = _animPlayMapping._nAnimClips;
				if (nAnimClips > 0)
				{
					for (int iUnit = 0; iUnit < nAnimClips; iUnit++)
					{
						...
						for (int iPV = 0; iPV < nValidParamKeyValues; iPV++)
						{
							...
							if (//!paramKeyValue._isCalculated ||//삭제 v1.4.7
							...
							//변경 v1.4.7 : VertexRequest에도 _isCalculated를 삭제하는 전략을 적용했다.
							vertRequest.OnModWeightPairCalculated(paramKeyValue._linkedVertexRequestMWP, paramKeyValue._weight);//그런데 이렇게 되면 iRealPKV가 필요 없넹?
...
//#endif	
		...
		private void Calculate_TF_UseModMeshSet_Animation(float tDelta)
		{
			...
			apOptBone rootMotionBone = null;
			apOptRootUnit rootUnit = _parentOptTransform._rootUnit;
			if(rootUnit != null
				&& _portrait._rootMotionValidatedMode == apPortrait.ROOT_MOTION_MODE.MoveParentTransform)
			{
				//루트 모션의 기준이 되는 루트 본
				rootMotionBone = _parentOptTransform._rootUnit.RootMotionBone;
			}
			...
			for (int i = 0; i < nCRP; i++)
			{
				...
				if (calParam._targetBone != null)
				{
					...
					//추가 v1.4.8
					//루트 모션용 "Mod에 의한 변화량"의 세부 정보를 Root Unit에 보고한다.
					//이걸 Parent 이동에 사용한다.
					//Bone의 구조상, 여기서의 Delta Pos가 다른 방향을 가리킬 수 있으므로, Bone World Matrix 연산을 꼭 참조해야한다.
					//Note - Prev_Delta > Cur_Delta 만큼만 이동해야하므로,
					//  만약 이전 프레임에 등록되지 않은 calParam(Animation)인 경우엔 Parent를 움직이지 않는다.
					//  이러면 첫 프레임의 위치가 반영되지 않아서 애니메이션이 바뀔때 원래 위치로 복구되는 문제가 해결될 것이다.
					isRootMotionBone = (rootMotionBone == calParam._targetBone);					
				}
				else
				{
					//ModMesh를 참조하는 Param이다.
					isBoneTarget = false;
					isRootMotionBone = false;
				}
				...
				if (nAnimClips > 0)
				{
					for (int iUnit = 0; iUnit < nAnimClips; iUnit++)
					{
						...
						//추가 v1.4.8 [루트 모션]
						//루트 모션의 루트 본에 대한 처리의 경우, tmpMatrix의 DeltaPos 부분의 값을 RootUnit에 전달한다.
						//DetaPos만큼 Root Parent를 이동시킬 것이다.
						...
						if (_curAnimLayeredParam._iCurAnimClip == 0)
						{
							...
							//v1.4.8 : 루트 모션
							//0번째 애니메이션 레이어에서만 동작한다.
							if (isRootMotionBone && _curAnimLayeredParam._layerIndex == 0)
							{
								rootUnit.AddRootMotionData(_keyAnimClip, ref tmpMatrix._pos, 1.0f);//시작은 Weight가 1
							}
						}
						else
						{
							...
							if (isRootMotionBone && _curAnimLayeredParam._layerIndex == 0)
							{
								rootUnit.AddRootMotionData(_keyAnimClip, ref tmpMatrix._pos, layerWeight);
							}
						}
		...
		private void Calculate_Rigging_UseModMeshSet(float tDelta)
		{
			...
			for (int iCalParam = 0; iCalParam < nCRP; iCalParam++)
			{
				calParam = _calculatedResultParams[iCalParam];
				...
				if(nSubParamGroups == 0)
				{
					calParam._isAvailable = true;//Rigging은 그냥 true
					continue;
				}
		...
		private void Calculate_Physics_UseModMeshSet(float tDelta)
		{
			...
			//[계산이 되는 유효 프레임 (Valid Frame)] - [이전 기록 유지 프레임]을 구분한다.
			if (_tDeltaFixed > PHYSIC_DELTA_TIME)
			{
				tDelta = PHYSIC_DELTA_TIME;
				_tDeltaFixed -= PHYSIC_DELTA_TIME;
			...
			//기존의 상대 위치를 이용하자
			//추가 v1.4.7 : 루트 유닛 변경시에도 텔레포트 프레임으로 인식
			if(_portrait._isCurrentTeleporting || _portrait._isCurrentRootUnitChanged)
			{
			...
			int nCalResultParams = _calculatedResultParams.Count;
			...
			for (int iCalParam = 0; iCalParam < nCalResultParams; iCalParam++)
			{
				calParam = _calculatedResultParams[iCalParam];
				...
				for (int iSubList = 0; iSubList < nSubParamGroups; iSubList++)
				{
					...
					for (int iPV = 0; iPV < nParamKeys; iPV++)
					{
						...
						//[v1.4.5] 삭제. 터치 입력 결과는 로컬 변수가 아닌 버텍스 개별에게 전달된다.
						//bool isTouchCalculated = false;
						//float touchCalculatedWeight = 0.0f;
						//Vector2 touchCalculatedDeltaPos = Vector2.zero;
						...
						for (int iPos = 0; iPos < tmpNumVert; iPos++)
						{
							...
							//변경 [v1.4.5]
							tmpModPhysicsVert._isExTouched = false;
							...
							if (isExtTouchProcessing)
							{
								...
								float pullTouchTotalWeight = 0.0f;
								//Weight를 새로 갱신하자
								for (int iTouch = 0; iTouch < apForceManager.MAX_TOUCH_UNIT; iTouch++)
								{
									apPullTouch touch = _portrait.GetTouch(iTouch);
									Vector2 touchPos = touch.Position;
									//touchPos *= -1;
									...
									if (isExtTouchWeightRefresh)
									{
										...
										{
											//pos 1F 위치에 의한 Weight를 새로 갱신해야한다.
											tmpModPhysicsVert._touchedWeight[iTouch] = touch.GetTouchedWeight(tmpModPhysicsVert._pos_1F);
											tmpModPhysicsVert._touchedPosDelta[iTouch] = tmpModPhysicsVert._pos_1F - touch.Position;
										}
										else
										{
											tmpModPhysicsVert._touchedWeight[iTouch] = -1.0f;//Weight를 초기화
										}
									}
									...
									if (touch.IsLive)
									{
										//Weight를 이용하여 보간을 하자
										//이후 누적 후 평균값을 넣자
										pullTouchPos += (tmpModPhysicsVert._touchedPosDelta[iTouch] + touch.Position - tmpModPhysicsVert._pos_1F) * tmpModPhysicsVert._touchedWeight[iTouch];
										pullTouchTotalWeight += tmpModPhysicsVert._touchedWeight[iTouch];
									}
								}
								...
								if (pullTouchTotalWeight > 0.0f)
								{
									...
									//변경 v1.4.5 : 개별 버텍스에 저장
									tmpModPhysicsVert._isExTouched = true;
									tmpModPhysicsVert._exTouchCalculatedDeltaPos = pullTouchPos;									
									tmpModPhysicsVert._exTouchWeight = itpPull;


									//추가 v1.4.5 : 터치가 발생하면 해당 버텍스의 관성은 없앤다.
									tmpModPhysicsVert.ClearInertiaByWeight(1.0f - itpPull);
								}
							}

							//v1.4.4 For 안에서 밖으로 위치 이동
							...
							if (isValidFrame)
							{
								...
								//[v1.4.5] 터치 개선
								//터치시 터치 가중치가 크면 힘이 0이 된다.
								if(tmpModPhysicsVert._isExTouched)
								{
									//(0.0 * TouchWeight) + (1.0 * (1 - TouchWeight)) > (1 - TouchWeight)
                                    tmpF_stretch *= 1.0f - tmpModPhysicsVert._exTouchWeight;
                                    tmpF_recover *= 1.0f - tmpModPhysicsVert._exTouchWeight;
                                }
								...
								//[v1.4.5] 터치 입력시 힘이 그만큼 줄어든다.
								//if (isTouchCalculated)
								//{
								//	tmpF_sum *= (1.0f - touchCalculatedWeight);
								//}

								//터치시 힘 감쇠. 가중치가 조금이라도 있으면 크게 감쇠하도록 x2배
								if (tmpModPhysicsVert._isExTouched)
								{
									tmpF_sum *= (1.0f - Mathf.Clamp01(tmpModPhysicsVert._exTouchWeight * 2.0f));
								}
							...
							if (isValidFrame)
							{
								...
								if (tmpModPhysicsVert._isExTouched)
								{
									limitedNextCalPos = (limitedNextCalPos * (1.0f - tmpModPhysicsVert._exTouchWeight)) + (tmpModPhysicsVert._exTouchCalculatedDeltaPos * tmpModPhysicsVert._exTouchWeight);
								...
								if (tmpModPhysicsVert._isExTouched)
								{
									Vector2 nextTouchPos = (limitedNextCalPos * (1.0f - tmpModPhysicsVert._exTouchWeight)) + (tmpModPhysicsVert._exTouchCalculatedDeltaPos * tmpModPhysicsVert._exTouchWeight);

									limitedNextCalPos = nextTouchPos;
									tmpNextVelocity = (limitedNextCalPos - tmpModPhysicsVert._calculatedDeltaPos) / tDelta;//변경

									//[v1.4.5] 2
									//속도도 Weight가 높을 수록 감속
									tmpNextVelocity *= (1.0f - tmpModPhysicsVert._exTouchWeight);
								}
						...
						for (int iPos = 0; iPos < tmpNumVert; iPos++)
						{
							...
							if (isValidFrame)
							{
                                ...
                                //[v1.4.5] 3 : 터치 로직이 있다면 > 계산 결과와 무관하게 속도가 감속되어야 한다.
                                if (tmpModPhysicsVert._isExTouched)
                                {
                                    tmpNextVelocity *= 1.0f - tmpModPhysicsVert._exTouchWeight;
                                }
		...
		public apOptCalculatedResultParam GetCalculatedResultParam(apOptTransform targetOptTransform)
		{
			...
			//변경 v1.5.0
			s_GetCalResultParam_TargetTransform = targetOptTransform;
			return _calculatedResultParams.Find(s_GetCalResultParamByTransform_Func);
		}
		...
		public apOptCalculatedResultParam GetCalculatedResultParam_Bone(apOptTransform targetOptTransform, apOptBone bone, apOptTransform ownerOptTransform)
		{
			...
			//return _calculatedResultParams.Find(delegate (apOptCalculatedResultParam a)
			//{
			//	return a._targetOptTransform == targetOptTransform
			//			&& a._targetBone == bone
			...
			s_GetCalResultParam_OwnerTransform = ownerOptTransform;

			return _calculatedResultParams.Find(s_GetCalResultParamByBone_Func);
		}


		private static apOptTransform s_GetCalResultParam_TargetTransform = null;
		private static apOptBone s_GetCalResultParam_Bone = null;
		private static apOptTransform s_GetCalResultParam_OwnerTransform = null;

		private static Predicate<apOptCalculatedResultParam> s_GetCalResultParamByTransform_Func = FUNC_GetCalResultParamByTransform;
		private static bool FUNC_GetCalResultParamByTransform(apOptCalculatedResultParam a)
		{
			return a._targetOptTransform == s_GetCalResultParam_TargetTransform && a._targetBone == null;
		}

		private static Predicate<apOptCalculatedResultParam> s_GetCalResultParamByBone_Func = FUNC_GetCalResultParamByBone;
		private static bool FUNC_GetCalResultParamByBone(apOptCalculatedResultParam a)
		{
			return a._targetOptTransform == s_GetCalResultParam_TargetTransform
						&& a._targetBone == s_GetCalResultParam_Bone
						&& a._ownerOptTransform == s_GetCalResultParam_OwnerTransform;
		}
	}
