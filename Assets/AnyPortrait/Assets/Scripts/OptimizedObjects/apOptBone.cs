...
namespace AnyPortrait
{
	...
	public class apOptBone : MonoBehaviour
	{
		...
		[NonSerialized]
		private Vector2 _calJig_EndPos_Excepted = Vector2.zero;

		[NonSerialized]
		private Vector2 _calJig_EndPos_LocalByShape = Vector2.zero;
		...
		public void Initialize(apPortrait parentPortrait)
		{
			...
			//지글본 계산 변수 일부 초기화
			_calJig_EndPos_LocalByShape = new Vector2(0.0f, _shapeLength);
			_calJig_EndPos_Excepted = Vector2.zero;
		}
					...
					)
				{
					...
					else
					{
							...
							)
						{
							...
							//변경 v1.4.4 : Ref를 이용하여 성능이 향상된 코드
			...
			if(_isJiggle && isPhysics)
			{
				...
				else
				{
						...
						)
					{
						...
						if (Mathf.Abs(_calJig_EndPos_Excepted.x - _calJig_EndPos_Prev.x) > 0.001f ||
							Mathf.Abs(_calJig_EndPos_Excepted.y - _calJig_EndPos_Prev.y) > 0.001f)
						{
							Vector2 start2End_Prev = _calJig_EndPos_Prev - _calJig_Tmp_WorldMatrix.Pos;
							Vector2 start2End_Expected = _calJig_EndPos_Excepted - _calJig_Tmp_WorldMatrix.Pos;
							...
							normalDeltaRatio = Mathf.Abs(Mathf.Sin(Vector2.Angle(start2End_Expected, _calJig_EndPos_Excepted - _calJig_EndPos_Prev) * Mathf.Deg2Rad));
					...
					//변경 v1.4.4 : Ref를 이용한 더 빠른 코드
					_worldMatrix.MulPoint2(ref _calJig_EndPos_Prev, ref _calJig_EndPos_LocalByShape);
		...
		private apMatrix3x3 _cal_WorldMatrix3x3 = apMatrix3x3.identity;//최적화용 계산 변수
		private apMatrix3x3 _cal_WorldMatrixNoModInv3x3 = apMatrix3x3.identity;//최적화용 계산 변수
		...
		public void UpdatePostRecursive()
		{
			...
			//임시 변수에 넣어서 계산 중에 값 복사를 줄인다. [v1.4.4]
			_cal_WorldMatrix3x3 = _worldMatrix.MtrxToSpace;
			_cal_WorldMatrixNoModInv3x3 = _worldMatrix_NonModified.MtrxToLowerSpace;
			...
			//이전
			//_shapePoint_End = _cal_WorldMatrix3x3.MultiplyPoint(_shapePoint_End);
			//_shapePoint_Mid1 = _cal_WorldMatrix3x3.MultiplyPoint(_shapePoint_Mid1);
			//_shapePoint_Mid2 = _cal_WorldMatrix3x3.MultiplyPoint(_shapePoint_Mid2);
			//_shapePoint_End1 = _cal_WorldMatrix3x3.MultiplyPoint(_shapePoint_End1);
			//_shapePoint_End2 = _cal_WorldMatrix3x3.MultiplyPoint(_shapePoint_End2);

			//변경 v1.4.4 : Ref 이용
			apMatrix3x3.MultiplyPoint(ref _shapePoint_End, ref _cal_WorldMatrix3x3, ref _shapePoint_End);
			apMatrix3x3.MultiplyPoint(ref _shapePoint_Mid1, ref _cal_WorldMatrix3x3, ref _shapePoint_Mid1);
			apMatrix3x3.MultiplyPoint(ref _shapePoint_Mid2, ref _cal_WorldMatrix3x3, ref _shapePoint_Mid2);
			apMatrix3x3.MultiplyPoint(ref _shapePoint_End1, ref _cal_WorldMatrix3x3, ref _shapePoint_End1);
			apMatrix3x3.MultiplyPoint(ref _shapePoint_End2, ref _cal_WorldMatrix3x3, ref _shapePoint_End2);
#endif
			...
			//변경 v1.4.4
			_vertWorld2BoneModWorldMatrix.SetAndMultiply(ref _cal_WorldMatrix3x3, ref _cal_WorldMatrixNoModInv3x3);
