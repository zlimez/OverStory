...
namespace AnyPortrait
{
	...
	public class apAnimTimelineLayer
	{
		...
		public void Init_TransformOfModifier(apAnimTimeline timeline, int uniqueID, int transformID, bool isMeshTransform, bool isOptionVividColor)
		{
			_uniqueID = uniqueID;
			...
			_guiColor = GetRandomColor(isOptionVividColor);
		}

		public void Init_ControlParam(apAnimTimeline timeline, int uniqueID, int controlParamID, bool isOptionVividColor)
		{
			_uniqueID = uniqueID;
		...
		public void Init_Bone(apAnimTimeline timeline, int uniqueID, int boneID, apBone srcBone, bool isOptionVividColor)
		{
			_uniqueID = uniqueID;
			...
			//v1.4.5 변경 : 본 색상을 최대한 활용한다. (약간의 바리에이션만 준다)
			if(srcBone != null)
			{
				//본 색상을 HSV로 환산한 후 > S와 V를 조금 조절해서 설정한다.
				float colHue = 0.0f;
				float colSat = 0.0f;
				float colVal = 0.0f;
				Color.RGBToHSV(srcBone._color, out colHue, out colSat, out colVal);

				//v1.4.6 : 옵션에 따라 Bone 색상을 HSV로 가공할 때의 값의 범위가 바뀐다.
				if(isOptionVividColor)
				{
					// 밝은 편
					colSat = UnityEngine.Random.Range(0.5f, 0.8f);
					colVal = UnityEngine.Random.Range(0.6f, 0.75f);
				}
				else
				{
					//어두운 편
					colSat = UnityEngine.Random.Range(0.2f, 0.4f);
					colVal = UnityEngine.Random.Range(0.3f, 0.4f);
				}
				

				_guiColor = Color.HSVToRGB(colHue, colSat, colVal);
			}
			else
			{
				_guiColor = GetRandomColor(isOptionVividColor);
		...
		private Color GetRandomColor(bool isVividColor)
		{
			//이전
			...
			//변경 v1.4.5 : HSV 랜덤을 이용하며 채도가 좀더 높아지도록 설정
			//변경 v1.4.6 : 옵션에 따라 값의 범위가 다름
			float randHue = 0.0f;
			float randSat = 0.0f;
			float randVal = 0.0f;
			...
			{
				//v1.4.6에서 밝기를 조금 낮춘다.
				randHue = UnityEngine.Random.Range(0.0f, 1.0f);//색조는 전체 랜덤
				randSat = UnityEngine.Random.Range(0.5f, 0.8f);//채도는 적정히
				randVal = UnityEngine.Random.Range(0.6f, 0.75f);//밝기가 높다
			}
			else
			{
				randHue = UnityEngine.Random.Range(0.0f, 1.0f);//색조는 전체 랜덤
				randSat = UnityEngine.Random.Range(0.2f, 0.4f);//채도가 조금 낮다.
				randVal = UnityEngine.Random.Range(0.3f, 0.4f);//밝기가 조금 낮다.
			}
			

			return Color.HSVToRGB(randHue, randSat, randVal);
