	...
	public partial class apSelection
	{
		...
		private void Draw_Overall(int width, int height)
		{
			...
			if (apEditorUtil.ToggledButton(Editor.ImageSet.Get(apImageSet.PRESET.Capture_Tab), 1, Editor.GetUIWord(UIWORD.Capture), isRootUnitTab_Capture, true, subTabWidth, subTabHeight, apStringFactory.I.CapturingTheScreenShot))//"Capturing the screenshot"
			{
				...
				{
					if (apVersion.I.IsDemo)//캡쳐 탭 사용 불가
					{
		...
		/// 다른 Width 프로퍼티들도 같이 변경된다. (Ref 붙은 값들이 변경된다)
		/// </summary>
		private void SetCaptureSizeFixedAspectRatio_WidthChanged(
															int prop1_Width_InputNext,
															ref int prop1_Width, 
															ref int prop1_Height,
															ref int prop2_Width, int prop2_Height,
															ref int prop3_Width, int prop3_Height)
		{
			//만약 입력값이 동일하다면 패스
			if(prop1_Width_InputNext == prop1_Width)
			{
				return;
			}

			if(prop1_Width_InputNext <= 8)
			{
				prop1_Width_InputNext = 8;
			}

			//기존의 AspectRatio
			float aspectRatio = 1.0f;
			if(prop1_Height > 0 && prop1_Width > 0)
			{
				aspectRatio = (float)prop1_Width / (float)prop1_Height;
			}

			//변경식
			//- 만약 변경 과정에서 배수가 정수인 경우 (x2, x3, /2 등), 그걸 그대로 대입한다.
			//- 만약 배수가 정수가 아니라면 aspectRatio를 이용한다.

			//aspectRatio = Width / Height;
			//Width = Height * aspectRatio
			//Height = Width / aspectRatio

			//Aspect Ratio를 따르도록 Height를 변경한다.
			bool isIntMultiplied = false;//<<정수배 연산이 실패한 경우에 실수 연산을 하자

			//정수배 연산이 실패한 경우
			if (prop1_Width > 0 && prop1_Width_InputNext > 0)
			{
				if (prop1_Width < prop1_Width_InputNext)
				{
					//더 큰값이 입력된 경우
					if (prop1_Width_InputNext % prop1_Width == 0)
					{
						//정수배 입력이다.
						int intMul = prop1_Width_InputNext / prop1_Width;

						//intMul만큼 크기가 확장되었으니, 동일한 값을 Height에 곱하자
						if (intMul > 0)
						{
							prop1_Height *= intMul;
							isIntMultiplied = true;
						}
					}
				}
				else
				{
					//더 작은 값이 입력된 경우
					if(prop1_Width % prop1_Width_InputNext == 0)
					{
						//정수배 (나누기) 입력이다.
						int intDiv = prop1_Width / prop1_Width_InputNext;

						//intDiv만큼 크기가 줄어들었으니, 동일한 값을 Height에 나누자
						//단, Height가 나누기 배수로 나누어 떨어져야 한다.
						if (intDiv > 0 && prop1_Height > intDiv && prop1_Height % intDiv == 0)
						{
							prop1_Height /= intDiv;
							if (prop1_Height < 1)
							{
			...
			if(!isIntMultiplied)
			{
				prop1_Height = Mathf.RoundToInt((float)prop1_Width_InputNext / aspectRatio);
			}
			
			//입력했던 Width 프로퍼티를 변경한다.
			prop1_Width = prop1_Width_InputNext;

			//연관된 다른 프로퍼티의 Width를 변경하자.
			//기준이 되는 Height를 이용하여 다음과 같이 변경한다.
			//- 원래 기준이 되는 Prop1의 Height와 Prop2, 3의 Height가 정수배 배수를 이룬다면, 해당 배수를 Prop2, 3의 Width에 그냥 적용한다.
			//- 그 외에는 "Width = Height / aspectRatio"를 이용한다.

			bool isIntMultiplied_2 = false;
			bool isIntMultiplied_3 = false;

			//Prop2 먼저
			if(prop2_Height > 0 && prop1_Height > 0)
			{
				if(prop2_Height == prop1_Height)
				{
					//같다면 > 동일하게 설정
					prop2_Width = prop1_Width;
					isIntMultiplied_2 = true;
				}
				else if(prop2_Height > prop1_Height)
				{
					//기준 Prop1 대비 확대를 하는 경우
					if(prop2_Height % prop1_Height == 0)
					{
						//정수배로 확대를 한다.
						int intMul = prop2_Height / prop1_Height;

						if(intMul > 0)
						{
							//Width도 Prop1 대비 정수배로 확대를 하자
							prop2_Width = prop1_Width * intMul;
							isIntMultiplied_2 = true;
						}
					}
				}
				...
				{
					//기준 Prop1 대비 축소를 하는 경우 (Prop2가 작다)
					if(prop1_Height % prop2_Height == 0)
					{
						//정수배로 축소를 한다.
						int intDiv = prop1_Height / prop2_Height;

						//배수로 나누어 떨어지는 경우에만
						if(intDiv > 0
							&& prop2_Width > intDiv
							&& prop2_Width % intDiv == 0)
						{
							//Width도 Prop1 대비 정수배로 축소를 하자
							prop2_Width = prop1_Width / intDiv;
							isIntMultiplied_2 = true;
						}
					}
				}
			}
			if(!isIntMultiplied_2)
			{
				//정수배 확대/축소가 아니므로
				//Width = Height * aspectRatio
				prop2_Width = Mathf.RoundToInt(prop2_Height * aspectRatio);
			}


			//Prop3 계산
			if(prop3_Height > 0 && prop1_Height > 0)
			{
				if(prop3_Height == prop1_Height)
				{
					//같다면 > 동일하게 설정
					prop3_Width = prop1_Width;
					isIntMultiplied_3 = true;
				}
				else if(prop3_Height > prop1_Height)
				{
					//기준 Prop1 대비 확대를 하는 경우
					if(prop3_Height % prop1_Height == 0)
					{
						//정수배로 확대를 한다.
						int intMul = prop3_Height / prop1_Height;

						if(intMul > 0)
						{
							//Width도 Prop1 대비 정수배로 확대를 하자
							prop3_Width = prop1_Width * intMul;
							isIntMultiplied_3 = true;
						}
					}
				}
				...
				{
					//기준 Prop1 대비 축소를 하는 경우 (Prop3가 작다)
					if(prop1_Height % prop3_Height == 0)
					{
						//정수배로 축소를 한다.
						int intDiv = prop1_Height / prop3_Height;

						//배수로 나누어 떨어지는 경우에만
						if(intDiv > 0
							&& prop3_Width > intDiv
							&& prop3_Width % intDiv == 0)
						{
							//Width도 Prop1 대비 정수배로 축소를 하자
							prop3_Width = prop1_Width / intDiv;
							isIntMultiplied_3 = true;
						}
					}
				}
			}
			if(!isIntMultiplied_3)
			{
				//정수배 확대/축소가 아니므로
				//Width = Height * aspectRatio
				prop3_Width = Mathf.RoundToInt(prop3_Height * aspectRatio);
			}
		}

		/// <summary>
		/// 캡쳐 옵션에서 크기가 바뀌는 경우에 Aspect Ratio 유지 옵션이 켜진 상태에서 Height가 변경되면 호출되는 함수.
		/// 다른 Height 프로퍼티들도 같이 변경된다. (Ref 붙은 값들이 변경된다)
		/// </summary>
		private void SetCaptureSizeFixedAspectRatio_HeightChanged(
															int prop1_Height_InputNext,															
															ref int prop1_Width,
															ref int prop1_Height,															
															int prop2_Width, ref int prop2_Height,
															int prop3_Width, ref int prop3_Height)
		{
			//만약 입력값이 동일하다면 패스
			if(prop1_Height_InputNext == prop1_Height)
			{
				return;
			}

			if(prop1_Height_InputNext <= 8)
			{
				prop1_Height_InputNext = 8;
			}
			...
			//정수배 연산이 실패한 경우
			if (prop1_Height > 0 && prop1_Height_InputNext > 0)
			{
				if (prop1_Height < prop1_Height_InputNext)
				{
					//더 큰값이 입력된 경우
					if (prop1_Height_InputNext % prop1_Height == 0)
					{
						//정수배 입력이다.
						int intMul = prop1_Height_InputNext / prop1_Height;

						//intMul만큼 크기가 확장되었으니, 동일한 값을 Width에 곱하자
						if (intMul > 0)
						{
							prop1_Width *= intMul;
							isIntMultiplied = true;
				...
				{
					//더 작은 값이 입력된 경우
					if(prop1_Height % prop1_Height_InputNext == 0)
					{
						//정수배 (나누기) 입력이다.
						int intDiv = prop1_Height / prop1_Height_InputNext;

						//intDiv만큼 크기가 줄어들었으니, 동일한 값을 Width에 나누자
						//단, Width가 나누기 배수로 나누어 떨어져야 한다.
						if (intDiv > 0 && prop1_Width > intDiv && prop1_Width % intDiv == 0)
						{
							prop1_Width /= intDiv;
							if (prop1_Width < 1)
							{
			...
			{
				//Width = Height * aspectRatio
				prop1_Width = Mathf.RoundToInt((float)prop1_Height_InputNext * aspectRatio);
			}
			
			//입력했던 Height 프로퍼티를 변경한다.
			prop1_Height = prop1_Height_InputNext;

			//연관된 다른 프로퍼티의 Height를 변경하자.
			//기준이 되는 Width를 이용하여 다음과 같이 변경한다.
			//- 원래 기준이 되는 Prop1의 Width와 Prop2, 3의 Width가 정수배 배수를 이룬다면, 해당 배수를 Prop2, 3의 Height에 그냥 적용한다.
			//- 그 외에는 "Height = Width / aspectRatio"를 이용한다.

			bool isIntMultiplied_2 = false;
			bool isIntMultiplied_3 = false;

			//Prop2 먼저
			if(prop2_Width > 0 && prop1_Width > 0)
			{
				if(prop2_Width == prop1_Width)
				{
					//같다면 > 동일하게 설정
					prop2_Height = prop1_Height;
					isIntMultiplied_2 = true;
				}
				else if(prop2_Width > prop1_Width)
				{
					//기준 Prop1 대비 확대를 하는 경우
					if(prop2_Width % prop1_Width == 0)
					{
						//정수배로 확대를 한다.
						int intMul = prop2_Width / prop1_Width;

						if(intMul > 0)
						{
							//Height도 Prop1 대비 정수배로 확대를 하자
							prop2_Height = prop1_Height * intMul;
							isIntMultiplied_2 = true;
						}
					}
				}
				...
				{
					//기준 Prop1 대비 축소를 하는 경우 (Prop2가 작다)
					if(prop1_Width % prop2_Width == 0)
					{
						//정수배로 축소를 한다.
						int intDiv = prop1_Width / prop2_Width;

						//배수로 나누어 떨어지는 경우에만
						if(intDiv > 0
							&& prop2_Height > intDiv
							&& prop2_Height % intDiv == 0)
						{
							//Height도 Prop1 대비 정수배로 축소를 하자
							prop2_Height = prop1_Height / intDiv;
							isIntMultiplied_2 = true;
						}
					}
				}
			...
			if(!isIntMultiplied_2)
			{
				//정수배 확대/축소가 아니므로
				//Height = Width / aspectRatio
				prop2_Height = Mathf.RoundToInt(prop2_Width / aspectRatio);
			}


			//Prop3 계산
			if(prop3_Width > 0 && prop1_Width > 0)
			{
				if(prop3_Width == prop1_Width)
				{
					//같다면 > 동일하게 설정
					prop3_Height = prop1_Height;
					isIntMultiplied_3 = true;
				}
				else if(prop3_Width > prop1_Width)
				{
					//기준 Prop1 대비 확대를 하는 경우
					if(prop3_Width % prop1_Width == 0)
					{
						//정수배로 확대를 한다.
						int intMul = prop3_Width / prop1_Width;

						if(intMul > 0)
						{
							//Width도 Prop1 대비 정수배로 확대를 하자
							prop3_Height = prop1_Height * intMul;
							isIntMultiplied_3 = true;
						}
					}
				}
				...
				{
					//기준 Prop1 대비 축소를 하는 경우 (Prop3가 작다)
					if(prop1_Width % prop3_Width == 0)
					{
						//정수배로 축소를 한다.
						int intDiv = prop1_Width / prop3_Width;

						//배수로 나누어 떨어지는 경우에만
						if(intDiv > 0
							&& prop3_Height > intDiv
							&& prop3_Height % intDiv == 0)
						{
							//Width도 Prop1 대비 정수배로 축소를 하자
							prop3_Height = prop1_Height / intDiv;
							isIntMultiplied_3 = true;
						}
					}
				}
			...
			if(!isIntMultiplied_3)
			{
				//정수배 확대/축소가 아니므로
				//Height = Width / aspectRatio
				prop3_Height = Mathf.RoundToInt(prop3_Width / aspectRatio);
			}
		}
		...
		private void StartMakeThumbnail()
		{
			...
			_captureLoadKey = newRequest.MakeScreenShot(OnThumbnailCaptured,
														...
														(int)((-1 * _editor._captureFrame_PosY) + apGL.WindowSizeHalf.y),//v1.4.6 : 부호 반전 (Y)
														srcThumbWidth, srcThumbHeight,
														thumbnailWidth, thumbnailHeight,
														Editor._scroll_CenterWorkSpace, Editor._iZoomX100,
														_editor._captureFrame_Color,
														_editor._captureFocusOffsetPX_X,
														_editor._captureFocusOffsetPX_Y,
														0, "");
		...
		private void StartTakeScreenShot()
		{
			try
			{
				...
				{
					int maxDstSizeWidth = 0;
					int maxDstSizeHeight = 0;

					apExporter.SIZE_CHECK_RESULT sizeResult = _editor.Exporter.IsDstSizeOverLimit(
																			...
																			out maxDstSizeWidth, out maxDstSizeHeight);

					if(sizeResult == apExporter.SIZE_CHECK_RESULT.SizeOver)
					{
						//최대 크기를 벗어났다고 알려주자
						string strMaxSize = "[" + maxDstSizeWidth + "x" + maxDstSizeHeight + "]";
						EditorUtility.DisplayDialog(	_editor.GetText(TEXT.DLG_CaptureOverMaxSize_Title),
														_editor.GetTextFormat(TEXT.DLG_CaptureOverMaxSize_Body, strMaxSize),
					...
					else if(sizeResult == apExporter.SIZE_CHECK_RESULT.SizeWarning)
					{
						//사이즈가 꽤 크다면, 처리 시간이 오래 걸리므로 선택하게 만든다.
						bool isContinue = EditorUtility.DisplayDialog(	_editor.GetText(TEXT.DLG_CaptureTooLargeImageSize_Title),
																		_editor.GetText(TEXT.DLG_CaptureTooLargeImageSize_Body),
																		_editor.GetText(TEXT.Okay),
				...
				string defFileName = "ScreenShot__" + DateTime.Now.Year + "_" + DateTime.Now.Month + "_" + DateTime.Now.Day + "_" + DateTime.Now.Hour + "_" + DateTime.Now.Minute + "_" + DateTime.Now.Second + ".png";
				...
				if (!string.IsNullOrEmpty(saveFilePath))
				{
					...
					_captureLoadKey = newRequest.MakeScreenShot(OnScreeenShotCaptured,
																...
																(int)(_editor._captureFrame_PosX + apGL.WindowSizeHalf.x),
																//(int)(_editor._captureFrame_PosY + apGL.WindowSizeHalf.y),
																(int)((-1 * _editor._captureFrame_PosY) + apGL.WindowSizeHalf.y),//v1.4.6 : Y 부호 반전
																_editor._captureFrame_SrcWidth, _editor._captureFrame_SrcHeight,
																_editor._captureFrame_DstWidth, _editor._captureFrame_DstHeight,
																Editor._scroll_CenterWorkSpace, Editor._iZoomX100,
																_editor._captureFrame_Color,
		...
		private void StartGIFAnimation()
		{
			...
			string defFileName = "GIF__" + DateTime.Now.Year + "_" + DateTime.Now.Month + "_" + DateTime.Now.Day + "_" + DateTime.Now.Hour + "_" + DateTime.Now.Minute + "_" + DateTime.Now.Second + ".gif";
		...
		private void StartMP4Animation()
		{
			...
			if(_editor._captureProcessor == apEditor.CAPTURE_PROCESSOR.Version2)
			{
				...
				apExporter.SIZE_CHECK_RESULT sizeResult = _editor.Exporter.IsDstSizeOverLimit(	_editor._captureFrame_SrcWidth, _editor._captureFrame_SrcHeight,
																		_editor._captureFrame_DstWidth, _editor._captureFrame_DstHeight,
																		out maxDstSizeWidth, out maxDstSizeHeight);
			...
			string defFileName = "MP4__" + DateTime.Now.Year + "_" + DateTime.Now.Month + "_" + DateTime.Now.Day + "_" + DateTime.Now.Hour + "_" + DateTime.Now.Minute + "_" + DateTime.Now.Second + ".mp4";
		...
		private void StartSpriteSheet(bool isSequenceFiles)
		{
			...
			if(_editor._captureProcessor == apEditor.CAPTURE_PROCESSOR.Version2)
			{
				...
				int targetDstSize_X = 0;
				int targetDstSize_Y = 0;

				if(isSequenceFiles)
				{
					//시퀸스 파일로 내보내는 경우
					targetDstSize_X = _editor._captureFrame_DstWidth;
					targetDstSize_Y = _editor._captureFrame_DstHeight;
				}
				else
				{
					//스프라이트 시트로 내보내는 경우
					targetDstSize_X = _editor._captureFrame_SpriteUnitWidth;
					targetDstSize_Y = _editor._captureFrame_SpriteUnitHeight;
				}

				apExporter.SIZE_CHECK_RESULT sizeResult = _editor.Exporter.IsDstSizeOverLimit(	_editor._captureFrame_SrcWidth, _editor._captureFrame_SrcHeight,
																		targetDstSize_X, targetDstSize_Y,
																		out maxDstSizeWidth, out maxDstSizeHeight);
			...
			if (!isSequenceFiles)
			{
				defFileName = "Spritesheet__" + DateTime.Now.Year + "_" + DateTime.Now.Month + "_" + DateTime.Now.Day + "_" + DateTime.Now.Hour + "_" + DateTime.Now.Minute + "_" + DateTime.Now.Second + ".png";
				saveFileDialogTitle = "Save Spritesheet";
			}
			else
			{
				defFileName = "Sequence__" + DateTime.Now.Year + "_" + DateTime.Now.Month + "_" + DateTime.Now.Day + "_" + DateTime.Now.Hour + "_" + DateTime.Now.Minute + "_" + DateTime.Now.Second + ".png";
		...
		private void Draw_ImageRes(int width, int height)
		{
			...
			//이미지 이름 변경 단축키
			Editor.AddHotKeyEvent(OnHotKeyEvent_RenameTextureData, apHotKeyMapping.KEY_TYPE.RenameObject, textureData);//v1.4.5 : 함수 위치 이동/내용 변경
		...
		private object _loadKey_RenameTextureData = null;

		/// <summary>
		/// 이미지의 이름 변경 이벤트
		/// </summary>
		private apHotKey.HotKeyResult OnHotKeyEvent_RenameTextureData(object paramObj)
		{
			if(SelectionType != SELECTION_TYPE.ImageRes
				|| TextureData == null
				|| paramObj == null)
			{
				return null;
			...
			apTextureData paramTextureData = paramObj as apTextureData;
			if(paramTextureData == null || paramTextureData != TextureData)
			{
				return null;
			}

			//변경 v1.4.5 : 다이얼로그를 띄운다.
			_loadKey_RenameTextureData = apDialog_Rename.ShowDialog(_editor, paramTextureData, null, paramTextureData._name, OnRenameResult_TextureData);

			return apHotKey.HotKeyResult.MakeResult();
		}


		//메시 이름 변경 다이얼로그 결과 [v1.4.5]
		public void OnRenameResult_TextureData(bool isSuccess, object loadKey, object targetObject, apEditorHierarchyUnit targetHierarchyUnit, string name)
		{
			if(!isSuccess
				|| loadKey == null
				|| _loadKey_RenameTextureData != loadKey
				|| targetObject == null
				|| string.IsNullOrEmpty(name)
				|| _editor == null
				|| _editor._portrait == null)
			{
				_loadKey_RenameTextureData = null;
				return;
			}
			_loadKey_RenameTextureData = null;

			apTextureData paramTextureData = targetObject as apTextureData;
			...
			apEditorUtil.SetRecord_Portrait(	apUndoGroupData.ACTION.MeshEdit_SettingChanged, 
												_editor, _editor._portrait,
												false,
												apEditorUtil.UNDO_STRUCT.ValueOnly);

			paramTextureData._name = name;

			//Rename후 객체를 선택하자[v1.4.2]
			if (Editor.Select.TextureData != paramTextureData)
			{
				Editor.Select.SelectImage(paramTextureData);
				if (Editor.Select.TextureData == paramTextureData)
				{
		...
		private void Draw_Mesh(int width, int height)
		{
			...
			//이름 변경 단축키 함수의 위치 이동 (탭 무관하게) v1.4.5.
			Editor.AddHotKeyEvent(OnHotKeyEvent_RenameMesh, apHotKeyMapping.KEY_TYPE.RenameObject, _mesh);//v1.4.5 : 함수 위치 이동/내용 변경
		...
		private void DrawMeshProperty_Setting(int width, int height)
		{
			...
			//이름 변경의 경우 탭에 상관없이 호출할 수 있다. [v1.4.5]
			//코드를 함수 바깥으로 옮긴다.
			////단축키 추가 (20.12.4)
			//Editor.AddHotKeyEvent(OnHotKeyEvent_RenameMesh, apHotKeyMapping.KEY_TYPE.RenameObject, _mesh);
		...
		{	
			...
			//[v1.4.5] 추가 : 선택한 MeshTF, MeshGroupTF의 이름을 바꿀 수 있다. (1개만 선택했을 경우 한정) 
			int nSelectedMeshTF = SubObjects.NumMeshTF;
			int nSelectedMeshGroupTF = SubObjects.NumMeshGroupTF;
			if(nSelectedMeshTF == 0 && nSelectedMeshGroupTF == 0)
			{
				//단축키 추가 (20.12.4) : 선택된게 없을땐, "메시 그룹"의 이름을 바꾼다.
				Editor.AddHotKeyEvent(OnHotKeyEvent_RenameMeshGroup, apHotKeyMapping.KEY_TYPE.RenameObject, _meshGroup);//단축키 처리 변경 [v1.4.5]
			}
			else if(nSelectedMeshTF == 1 && nSelectedMeshGroupTF == 0)
			{
				//[v1.4.5] MeshTF 한개만 선택했다면
				if(SubObjects.MeshTF != null)
				{
					//파라미터를 MeshTF로 넣자
					Editor.AddHotKeyEvent(OnHotKeyEvent_RenameMeshGroup, apHotKeyMapping.KEY_TYPE.RenameObject, SubObjects.MeshTF);//단축키 처리 변경 [v1.4.5]
				}
			}
			else if(nSelectedMeshGroupTF == 1 && nSelectedMeshTF == 0)
			{
				//[v1.4.5] MeshGroup TF를 한개만 선택했다면
				if(SubObjects.MeshGroupTF != null)
				{
					//파라미터를 MeshGroupTF로 넣자
					Editor.AddHotKeyEvent(OnHotKeyEvent_RenameMeshGroup, apHotKeyMapping.KEY_TYPE.RenameObject, SubObjects.MeshGroupTF);//단축키 처리 변경 [v1.4.5]
				}
			}
				...
				_isMeshGroupSetting_EditDefaultTransform, true, width, 30, apStringFactory.I.EditDefaultTransformsOfSubMeshesMeshGroups))//"Edit Default Transforms of Sub Meshs/MeshGroups"
			{
				...
				{
					//v1.4.4 : 편집이 종료되면 Render Vertex의 World Pos 중 일부 값을 갱신해야 한다.
					if (_meshGroup._rootRenderUnit != null)
					{
						_meshGroup._rootRenderUnit.CalculateRenverVertsWorldPosNoMod(apRenderUnit.CALCULATE_NO_MOD_POS_REQUEST.RefreshWorldMatrix_WithChild);
					}
				}
			}
			...
			{
				//복사는 데모에서 불가능하다.
				if (apVersion.I.IsDemo)// 메시 그룹 복제 불가
				{
				...
				else
				{
					...
					Editor.Controller.DuplicateMeshGroup(MeshGroup, null, true, true);
		...
		private void Draw_Param(int width, int height)
		{
			...
			//추가 v1.4.5
			bool next_IsAsIndex = cParam._isNonInterpolatedIndex;
			...
			//단축키 추가 (20.12.4)
			Editor.AddHotKeyEvent(OnHotKeyEvent_RenameControlParam, apHotKeyMapping.KEY_TYPE.RenameObject, Param);//v1.4.5 : 다이얼로그가 나오도록 변경
			...
			//추가 v1.4.5 : 보간이 안되는 Index 타입의 Int형 파라미터
			if(cParam._valueType == apControlParam.TYPE.Int)
			{
				GUILayout.Space(10);
				apEditorUtil.GUI_DelimeterBoxH(width);
				GUILayout.Space(10);

				//Non-Interpolated Index
				EditorGUILayout.LabelField(Editor.GetUIWord(UIWORD.IntProcessingMethod));
				int iPrevIntType = cParam._isNonInterpolatedIndex ? 1 : 0;
				int iNextIntType = EditorGUILayout.Popup(iPrevIntType, _controlParam_IntegerTypeNames);
				//next_IsAsIndex = EditorGUILayout.Toggle(Editor.GetUIWord(UIWORD.IndexType), cParam._isNonInterpolatedIndex);
				next_IsAsIndex = iNextIntType == 1;
			}
			...
			if (EditorGUI.EndChangeCheck())
			{
				if (next_category != cParam._category ||
					...
					!string.Equals(next_label_Min, cParam._label_Min) ||
					!string.Equals(next_label_Max, cParam._label_Max) ||
					next_snapSize != cParam._snapSize ||

					next_int_Def != cParam._int_Def ||
					Mathf.Abs(next_float_Def - cParam._float_Def) > 0.001f ||
					Mathf.Abs(next_vec2_Def.x - cParam._vec2_Def.x) > 0.001f ||
					Mathf.Abs(next_vec2_Def.y - cParam._vec2_Def.y) > 0.001f ||

					next_int_Min != cParam._int_Min ||
					next_int_Max != cParam._int_Max ||

					Mathf.Abs(next_float_Min - cParam._float_Min) > 0.001f ||
					Mathf.Abs(next_float_Max - cParam._float_Max) > 0.001f ||

					Mathf.Abs(next_vec2_Min.x - cParam._vec2_Min.x) > 0.01f ||
					Mathf.Abs(next_vec2_Min.y - cParam._vec2_Min.y) > 0.01f ||
					Mathf.Abs(next_vec2_Max.x - cParam._vec2_Max.x) > 0.01f ||
					Mathf.Abs(next_vec2_Max.y - cParam._vec2_Max.y) > 0.01f ||

					next_IsAsIndex != cParam._isNonInterpolatedIndex
					)
				{
					...
					cParam._isNonInterpolatedIndex = next_IsAsIndex;
		...
		private void DrawEditor_Right2_MeshGroup_Setting(int width, int height)
		{
			...
			if (isRender_Selected)
			{
				...
				if (isRender_SingleMeshTF || isRender_SingleMeshGroupTF)
				{
					...
					//> [v1.4.5] 삭제 : F2를 눌렀을 때 다이얼로그가 나오도록 변경됨
					//if (!isDummy)
					//{
					//	if (MeshTF_Main != null)
					//	{
					...
					))
				{
					...
					{
						//복사하기 (데모에선 복제가 막혀있다.)
						if (apVersion.I.IsDemo)//서브 객체 복제 불가
						{
							//데모 제한 다이얼로그
							EditorUtility.DisplayDialog(	Editor.GetText(TEXT.DemoLimitation_Title),
															Editor.GetText(TEXT.DemoLimitation_Body),
															Editor.GetText(TEXT.Okay));
						}
						else
					...
					if (isMeshTransform && (isRender_SingleMeshTF || isRender_MultiMeshTF) && MeshTF_Main != null)
					{
						if (!isDummy)
						{
							...
							{
								//이주 기능은 데모에서는 불가능하다
								if (apVersion.I.IsDemo)//서브메시/메시그룹 이주 기능
								{
									//데모 제한 다이얼로그
									EditorUtility.DisplayDialog(	Editor.GetText(TEXT.DemoLimitation_Title),
																	Editor.GetText(TEXT.DemoLimitation_Body),
		...
		private void DrawEditor_Right2_MeshGroup_Bone(int width, int height)
		{
			...
			if (isRender_Single)
			{
				...
				//이름 바꾸기 단축키 (21.2.9)
				Editor.AddHotKeyEvent(OnHotKeyEvent_RenameBone, apHotKeyMapping.KEY_TYPE.RenameObject, curBone);//v1.4.5 : 이름 변경 다이얼로그가 나오도록
			}
			else
			{
				//[다중 선택]
			...
			if (isRender_Single)
			{
				...
				{
					if (apVersion.I.IsDemo)//지글본 버튼 비활성
					{
						//데모 제한 다이얼로그
						EditorUtility.DisplayDialog(
						Editor.GetText(TEXT.DemoLimitation_Title),
			...
			if (isRender_Single)
			{
				...
				if (curBone._optionIK != apBone.OPTION_IK.Disabled)
				{
					...
					if (curBone._isIKAngleRange)
					{
						...
						//EditorGUILayout.BeginHorizontal(apGUILOFactory.I.Width(width));
						//EditorGUILayout.LabelField(Editor.GetUIWord(UIWORD.Preferred), apGUILOFactory.I.Width(100));//"Preferred"
						//float nextPreferredAngle = EditorGUILayout.DelayedFloatField(curBone._IKAnglePreferred, apGUILOFactory.I.Width(width - (110 + 54)));//<<정밀한 작업을 위해서 변경

						////Auto 버튼 누르면 Range의 평균값을 가지도록 설정
						//if(GUILayout.Button(Editor.GetUIWord(UIWORD.Auto), apGUILOFactory.I.Width(50)))
						//{
						//	nextPreferredAngle = (nextLowerAngle + nextUpperAngle) * 0.5f;
						//}
				...
				if (nextIKControllerType != curBone._IKController._controllerType)
				{
					...
					if (apVersion.I.IsDemo)//IK 컨트롤러 설정 제한
					{
						//데모 제한 다이얼로그
						EditorUtility.DisplayDialog(	Editor.GetText(TEXT.DemoLimitation_Title),
														Editor.GetText(TEXT.DemoLimitation_Body),
		...
		{	
											...
											false, isPosCopyAvailable, width2Btn, heightToolBtn))
			{
				...
				if(ModMesh_Main != null && ModMesh_Main._renderUnit != null)
				{
					...
					ModMesh_Main._renderUnit.CalculateRenverVertsWorldPosNoMod(apRenderUnit.CALCULATE_NO_MOD_POS_REQUEST.RefreshWorldMatrix_Single);
...
#region [미사용 코드] 더이상 선택된 객체 정보를 출력하지 않는다. (다중 선택도 있고 불필요하다고 판단) 20.6.12
		...
		private void DrawEditor_Bottom_Animation(int width, int height, int layoutX, int layoutY, int windowWidth, int windowHeight)
		{
			...
			for (int iLayer = 0; iLayer < nTimelineInfoList; iLayer++)
			{
				...
				if (info._isRenderable)
				{
					...
					else
					{
						...
						//변경 [v1.4.5]
						float grayScale = (layerBGColor_Default.r * 0.3f) + (layerBGColor_Default.g * 0.6f) + (layerBGColor_Default.b * 0.1f);						
						//if (grayScale < 0.5f)
						//{
						//	textColorType = 1;
						//}

						//다시 변경 [v1.4.6] 주로 흰색을 보여주며, 아주 밝은 경우에만 검은색이 나오게 하자
						if (grayScale < 0.8f)//밝기가 어느정도 선을 넘지 않는다면 흰색
						{
							textColorType = 1;
						}
					}
			...
			for (int iLayer = 0; iLayer < nTimelineInfoList; iLayer++)
			{
				...
				if (info._isSelected && isRepaintEvent)
				{
					apTimelineGL.DrawTimeBars_MainBG(	info.GetTimelineColor(isSelectedFlashingColor),
														curLayerY + layerHeight - (int)_scroll_Timeline.y,
														layerHeight);
				}
				curLayerY += layerHeight;
			}
			...
			//Grid를 그린다.
			//[v1.4.5] Repaint 이벤트일 때만 그리도록 해야한다.
			if (isRepaintEvent)
			{
			...
			for (int iLayer = 0; iLayer < nTimelineInfoList; iLayer++)
			{
				...
				if (info._isRenderable)
				{
					if(isRepaintEvent)//[v1.4.5] Repaint에서만 동작하도록 제한
					{
			...
			apTimelineGL.DrawKeySummry(_subAnimCommonKeyframeList, 58, isRepaintEvent);

			//Onion 마커를 그린다.
			if (Editor.Onion.IsVisible
				&& isRepaintEvent//[v1.4.5] Repaint 이벤트에서만
				)
			{
			...
			bool isChangeFrame = apTimelineGL.DrawPlayBar(AnimClip.CurFrame, isRepaintEvent);

			//변경 22.7.2 : PlayBar 앞에서 그려지도록 변경되었다.
			apTimelineGL.DrawEventMarkers(_animClip._animEvents, 30, isRepaintEvent);
			...
			if (isKeyframeEvent)
			{
				...
				//[v1.4.5]
				//만약 키프레임 등의 뭔가를 선택했을 때
				//"마지막으로 클릭한 타임라인 레이어"가 갱신될 필요가 있는지 확인한다.
				if(_lastClickTimelineLayer == null)
				{
					//선택된게 없을 때
					if(AnimTimelineLayer_Main != null)
					{
						//Debug.Log("클릭한 레이어 갱신 (키프레임 선택)");
						_lastClickTimelineLayer = AnimTimelineLayer_Main;//메인 레이어를 마지막 클릭으로 간주
					}
				}
				else
				{
					...
					{
						//선택된 레이어가 없다면
						//Debug.Log("클릭한 레이어 갱신 > 해제 (키프레임 선택)");
						_lastClickTimelineLayer = null;//선택 해제
					}
					else
					{
						//기존의 선택된 레이어 리스트에 포함되었는지 확인
						if(!AnimTimelineLayers_All.Contains(_lastClickTimelineLayer))
						{
							//현재 선택된 레이어에 마지막 클릭 레이어가 없다면
							//클릭한 레이어 갱신
							//Debug.Log("클릭한 레이어 갱신 (키프레임 선택)");
							_lastClickTimelineLayer = AnimTimelineLayer_Main;
						}
					}
			...
			apTimelineGL.DrawAndUpdateSelectArea(isRepaintEvent);
		...
		private void DrawEditor_Bottom_AnimationProperty_SingleKeyframe(apAnimKeyframe keyframe, int width, int windowWidth, int windowHeight, int layoutX, int layoutY, int scrollValue)
		{
			...
			GUILayout.Space(5);
			//"Transform"
			if (apEditorUtil.ToggledButton(isControlParamUI ? Editor.GetUIWord(UIWORD.ControlParameter) : Editor.GetUIWord(UIWORD.Transform),
...
#region Control Param UI 그리는 코드
					...
					//pasteResetTargets |= apDialog_SetValueTarget.SELECTABLE_TARGETS.
				...
				if (isDrawModifierUI && isSameKP)
				{
					...
					if (isMod_TF)
					{
						...
						if (isGUIChanged)
						{
							...
							if (modMesh != null)
							{
								if (Mathf.Abs(nextPos.x - modMesh._transformMatrix._pos.x) > bias ||
									Mathf.Abs(nextPos.y - modMesh._transformMatrix._pos.y) > bias ||
									Mathf.Abs(nextAngle - modMesh._transformMatrix._angleDeg) > bias ||
									Mathf.Abs(nextScale.x - modMesh._transformMatrix._scale.x) > bias ||
									Mathf.Abs(nextScale.y - modMesh._transformMatrix._scale.y) > bias
									)
								{
									isChanged = true;
							...
							{
								if (Mathf.Abs(nextPos.x - modBone._transformMatrix._pos.x) > bias ||
									Mathf.Abs(nextPos.y - modBone._transformMatrix._pos.y) > bias ||
									Mathf.Abs(nextAngle - modBone._transformMatrix._angleDeg) > bias ||
									Mathf.Abs(nextScale.x - modBone._transformMatrix._scale.x) > bias ||
									Mathf.Abs(nextScale.y - modBone._transformMatrix._scale.y) > bias
									)
								{
									isChanged = true;
					...
					if (isMod_Color)
					{
						...
						if (linkedModifier._isColorPropertyEnabled)
						{
							...
							{
								float bias = 0.001f;
								if (Mathf.Abs(nextColor.r - modMesh._meshColor.r) > bias ||
									Mathf.Abs(nextColor.g - modMesh._meshColor.g) > bias ||
									Mathf.Abs(nextColor.b - modMesh._meshColor.b) > bias ||
									Mathf.Abs(nextColor.a - modMesh._meshColor.a) > bias ||
			...
			if (isSameKP)
			{
				...
				{
					bool isExecutable = Editor.CheckModalAndExecutable();
					if (keyframe != null && isExecutable)
					{
						//붙여넣기
						//Anim (portrait) + Keyframe+LinkedMod (Modifier = nullable)

						//컨트롤 파라미터나 본을 대상으로 하는 경우엔 직접 변경
						//메시를 대상으로 한다면 속성 물어보기
						bool isDirectPaste = false;
						bool isShowPropDialog = false;

						if (isControlParamUI && controlParam != null)
						{
							//컨트롤 파라미터인 경우 > 직접 복사
							isDirectPaste = true;
						}
						else if (isModifierUI && parentTimeline._linkedModifier != null)
						{
							//모디파이어 타입이라면
							//- 대상이 메시인 경우 리셋 다이얼로그가 나와야 한다.
							//- 대상이 본인 경우 바로 리셋을 한다 (리셋 속성이 하나 뿐이라서)

							if (keyframe._linkedModMesh_Editor != null)
							{
								isShowPropDialog = true;
							}
						...
						else if (isShowPropDialog)
						{
							//속성을 선택하여 붙여넣기
							if (parentTimeline._linkedModifier != null)
							{
								//메시는 속성이 많아서 다이얼로그가 나온다.
								apDialog_SetValueTarget.SELECTABLE_TARGETS targetProps = apDialog_SetValueTarget.SELECTABLE_TARGETS.None;

								apModifierBase linkedMod = parentTimeline._linkedModifier;

								switch (linkedMod.ModifierType)
								{
									case apModifierBase.MODIFIER_TYPE.AnimatedMorph:
										targetProps = apDialog_SetValueTarget.SELECTABLE_TARGETS.Vertices
													| apDialog_SetValueTarget.SELECTABLE_TARGETS.Pins
													| apDialog_SetValueTarget.SELECTABLE_TARGETS.Visibility
													| apDialog_SetValueTarget.SELECTABLE_TARGETS.Color
													| apDialog_SetValueTarget.SELECTABLE_TARGETS.Extra;
									...
									case apModifierBase.MODIFIER_TYPE.AnimatedTF:
										targetProps = apDialog_SetValueTarget.SELECTABLE_TARGETS.Transform
													| apDialog_SetValueTarget.SELECTABLE_TARGETS.Visibility
													| apDialog_SetValueTarget.SELECTABLE_TARGETS.Color
													| apDialog_SetValueTarget.SELECTABLE_TARGETS.Extra;
										break;
									...
									case apModifierBase.MODIFIER_TYPE.AnimatedColorOnly:
										targetProps = apDialog_SetValueTarget.SELECTABLE_TARGETS.Visibility
													| apDialog_SetValueTarget.SELECTABLE_TARGETS.Color
													| apDialog_SetValueTarget.SELECTABLE_TARGETS.Extra;
										break;
								...
								_loadKey_AnimKeyframe_SetValue = apDialog_SetValueTarget.ShowDialog(
																		Editor,
																		AnimClip._targetMeshGroup,
																		linkedMod,
																		targetProps,
																		keyframe,
																		OnKeyframeKeyframeValue_Paste,
																		"Paste Value");
							}
						}
				...
				//v1.4.5 리셋 버튼
				if(GUILayout.Button(_editor.GetUIWord(UIWORD.ResetValue), apGUILOFactory.I.Height(20)))
				{
					if (keyframe != null)
					{
						...
						if (isExecutable)
						{
							if(isControlParamUI && controlParam != null)
							{
								...
								{
									case apControlParam.TYPE.Int:
										keyframe._conSyncValue_Int = controlParam._int_Def;
										break;

									case apControlParam.TYPE.Float:
										keyframe._conSyncValue_Float = controlParam._float_Def;
										break;

									case apControlParam.TYPE.Vector2:
										keyframe._conSyncValue_Vector2 = controlParam._vec2_Def;
										break;
								}
							...
							else if(isModifierUI && parentTimeline._linkedModifier != null)
							{
								...
								else if(keyframe._linkedModBone_Editor != null)
								{
									...
									//본은 Transform 초기화만 한다.
									modBone._transformMatrix.SetIdentity();

									AnimClip._targetMeshGroup.RefreshForce();
									Editor.RefreshControllerAndHierarchy(false);
								}
		...
		//추가 22.7.9 : 컨트롤 파라미터-모디파이어의 ParamSet의 값을 리셋할 때, 대상의 일부만 리셋할 수 있다.
		private object _loadKey_AnimKeyframe_SetValue = null;
		private void OnKeyframeKeyframeValue_Reset(	bool isSuccess,
														object loadKey, 
														apMeshGroup meshGroup,
														apModifierBase modifier,
														apDialog_SetValueTarget.SELECTABLE_TARGETS selectedTargets, 
		...
		{
			if(!isSuccess
				|| _loadKey_AnimKeyframe_SetValue != loadKey
				|| loadKey == null
				|| meshGroup == null
				|| modifier == null
				|| AnimClip == null)
			{
				_loadKey_AnimKeyframe_SetValue = null;
			...
			//저장된 값은 키프레임이다.
			apAnimKeyframe keyframe = savedObject as apAnimKeyframe;
			if(keyframe == null)
			{
				return;
			}
			...
			if(modMesh != null)
			{
				modMesh.ResetValues(	isResetVerts,
										isResetPins,
										isResetTransform,
										isResetVisibility,
										isResetColor,
			...
			if(modBone != null && isResetTransform)
			{
				//본은 Transform 초기화만 한다.
				modBone._transformMatrix.SetIdentity();
			}
		...
		//붙여넣기
		private void OnKeyframeKeyframeValue_Paste(bool isSuccess,
														object loadKey,
														apMeshGroup meshGroup,
														apModifierBase modifier,
														apDialog_SetValueTarget.SELECTABLE_TARGETS selectedTargets,
														...
														int nSelectedSlots)
		{
			...
			//선택한 속성 붙여넣기
			bool isPasteVerts =			(int)(selectedTargets & apDialog_SetValueTarget.SELECTABLE_TARGETS.Vertices) != 0;
			bool isPastePins =			(int)(selectedTargets & apDialog_SetValueTarget.SELECTABLE_TARGETS.Pins) != 0;
			bool isPasteTransform =		(int)(selectedTargets & apDialog_SetValueTarget.SELECTABLE_TARGETS.Transform) != 0;
			bool isPasteVisibility =	(int)(selectedTargets & apDialog_SetValueTarget.SELECTABLE_TARGETS.Visibility) != 0;
			bool isPasteColor =			(int)(selectedTargets & apDialog_SetValueTarget.SELECTABLE_TARGETS.Color) != 0;
			bool isPasteExtra =			(int)(selectedTargets & apDialog_SetValueTarget.SELECTABLE_TARGETS.Extra) != 0;
			...
			apSnapShotManager.I.Paste_Keyframe_MeshWithProps(	keyframe,
																isPasteVerts,
																isPastePins,
																isPasteTransform,
																isPasteVisibility,
