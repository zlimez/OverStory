...
namespace AnyPortrait
{
	...
	public partial class apSelection
	{
		...
		private object _loadKey_RenameMesh = null;
		...
		private apHotKey.HotKeyResult OnHotKeyEvent_RenameMesh(object paramObj)
		{
			if(SelectionType != SELECTION_TYPE.Mesh
				//|| Editor._meshEditMode != apEditor.MESH_EDIT_MODE.Setting//<<조건 삭제 v1.4.5
				|| Mesh == null
				|| paramObj == null)
			{
				return null;
			...
			apMesh paramMesh = paramObj as apMesh;
			if(paramMesh == null || paramMesh != Mesh)
			{
				return null;
			}
			...
			_loadKey_RenameMesh = apDialog_Rename.ShowDialog(_editor, _mesh, null, _mesh._name, OnRenameResult_Mesh);
		...
		public void OnRenameResult_Mesh(bool isSuccess, object loadKey, object targetObject, apEditorHierarchyUnit targetHierarchyUnit, string name)
		{
			if(!isSuccess
				|| loadKey == null
				|| _loadKey_RenameMesh != loadKey
				|| targetObject == null
				|| string.IsNullOrEmpty(name)
				|| _editor == null
				|| _editor._portrait == null)
			{
				_loadKey_RenameMesh = null;
				return;
			}
			_loadKey_RenameMesh = null;
		...
		private object _loadKey_RenameMeshGroupOrTF = null;
		...
		{
			//[v1.4.5] 변경
			//입력 값으로 MeshGroup 뿐만 아니라 MeshTF 또는 MeshGroupTF가 올 수 있다.
			...
			//[이전] 이름바꾸기 칸으로 포커스를 옮기자
			//apEditorUtil.SetGUIFocus_TextField(apStringFactory.I.GUI_ID__MeshGroupName);

			bool isProcessed = false;
			if(paramObj is apMeshGroup)
			{
				//메시 그룹 이름 바꾸기
				apMeshGroup targetMeshGroup = paramObj as apMeshGroup;
				if(targetMeshGroup != null)
				{
					_loadKey_RenameMeshGroupOrTF = apDialog_Rename.ShowDialog(	_editor,
																				targetMeshGroup,
																				null,
																				targetMeshGroup._name,
																				OnRenameResult_MeshGroup);
					isProcessed = true;
				}
			}
			else if(paramObj is apTransform_Mesh)
			{
				//메시 TF 이름 바꾸기
				apTransform_Mesh targetMeshTF = paramObj as apTransform_Mesh;
				if(targetMeshTF != null)
				{
					_loadKey_RenameMeshGroupOrTF = apDialog_Rename.ShowDialog(	_editor,
																				targetMeshTF,
																				null,
																				targetMeshTF._nickName,
																				OnRenameResult_MeshGroup);
					isProcessed = true;
				}
			}
			else if(paramObj is apTransform_MeshGroup)
			{
				apTransform_MeshGroup targetMeshGroupTF = paramObj as apTransform_MeshGroup;
				if(targetMeshGroupTF != null)
				{
					_loadKey_RenameMeshGroupOrTF = apDialog_Rename.ShowDialog(	_editor,
																				targetMeshGroupTF,
																				null,
																				targetMeshGroupTF._nickName,
																				OnRenameResult_MeshGroup);
					isProcessed = true;
				}
			}
		...
		//메시 그룹 또는 TF 이름 변경 다이얼로그 결과 [v1.4.5]
		public void OnRenameResult_MeshGroup(bool isSuccess, object loadKey, object targetObject, apEditorHierarchyUnit targetHierarchyUnit, string name)
		{
			if(!isSuccess
				|| loadKey == null
				|| _loadKey_RenameMeshGroupOrTF != loadKey
				|| targetObject == null
				|| string.IsNullOrEmpty(name)
				|| _editor == null
				|| _editor._portrait == null)
			{
				_loadKey_RenameMeshGroupOrTF = null;
				return;
			}
			_loadKey_RenameMeshGroupOrTF = null;

			//타입은 메시 그룹, 메시 TF, 메시 그룹 TF 중 하나이다.
			if(targetObject is apMeshGroup)
			{
				apMeshGroup targetMeshGroup = targetObject as apMeshGroup;
				
				if(targetMeshGroup != null)
				{
					Editor.Controller.RenameMeshGroup(targetMeshGroup, name);
				}
				
			}
			else if(targetObject is apTransform_Mesh)
			{	
				apTransform_Mesh targetMeshTF = targetObject as apTransform_Mesh;
				if(targetMeshTF != null)
				{	
					apEditorUtil.SetRecord_MeshGroup(	apUndoGroupData.ACTION.MeshGroup_DefaultSettingChanged, 
														...
														false, true,
														apEditorUtil.UNDO_STRUCT.ValueOnly);
					targetMeshTF._nickName = name;
				}
				
			}
			else if(targetObject is apTransform_MeshGroup)
			{
				apTransform_MeshGroup targetMeshGroupTF = targetObject as apTransform_MeshGroup;
				if(targetMeshGroupTF != null)
				{
					apEditorUtil.SetRecord_MeshGroup(	apUndoGroupData.ACTION.MeshGroup_DefaultSettingChanged, 
														Editor, 
														...
														false, true,
														apEditorUtil.UNDO_STRUCT.ValueOnly);
					targetMeshGroupTF._nickName = name;
				}
			}
		...
		private apHotKey.HotKeyResult OnHotKeyEvent_RenameBone(object paramObj)
		{
			...
			//변경 v1.4.5
			apBone targetBone = paramObj as apBone;
			bool isProcessed = false;
			if(targetBone != null)
			{
				_loadKey_RenameBone = apDialog_Rename.ShowDialog(	_editor,
																	targetBone,
																	null,
																	targetBone._name,
																	OnRenameResult_RenameBone);
				isProcessed = true;
			}
		...
		private object _loadKey_RenameBone = null;
		public void OnRenameResult_RenameBone(bool isSuccess, object loadKey, object targetObject, apEditorHierarchyUnit targetHierarchyUnit, string name)
		{
			if (!isSuccess
				|| loadKey == null
				|| _loadKey_RenameBone != loadKey
				|| targetObject == null
				|| string.IsNullOrEmpty(name)
				|| _editor == null
				|| _editor._portrait == null)
			{
				_loadKey_RenameBone = null;
				return;
			}
			_loadKey_RenameBone = null;

			if(targetObject is apBone)
			{
		...
		private object _loadKey_RenameAnimClip = null;
		...
		private apHotKey.HotKeyResult OnHotKey_RenameAnimClip(object paramObject)
		{
			...
			//[v1.4.5] 변경
			_loadKey_RenameAnimClip = apDialog_Rename.ShowDialog(_editor, AnimClip, null, AnimClip._name, OnRenameResult_AnimClip);

			return apHotKey.HotKeyResult.MakeResult();
		}


		public void OnRenameResult_AnimClip(bool isSuccess, object loadKey, object targetObject, apEditorHierarchyUnit targetHierarchyUnit, string name)
		{
			if(!isSuccess
				|| loadKey == null
				|| _loadKey_RenameAnimClip != loadKey
				|| targetObject == null
				|| string.IsNullOrEmpty(name)
				|| _editor == null
				|| _editor._portrait == null)
			{
				_loadKey_RenameAnimClip = null;
				return;
			}
		...
		private void OnSelectMeshGroupToAnimClip(bool isSuccess, object loadKey, apMeshGroup meshGroup, apAnimClip targetAnimClip)
		{
			...
			{
				//추가 v1.4.5
				meshGroup._modifierStack.EnableAllModifierAndPSGs();
			...
			if (isChanged)
			{
				...
				if (_animClip._targetMeshGroup != null)
				{
					...
					//추가 v1.4.4
					//다 계산한 World Matrix 중 No-Mod에 의한 위치값이 업데이트 순서 문제로 RenderVertex에 반영되지 않는 문제
					//[계산한 World Matrix 적용 + 모든 자식 갱신] 옵션
					if(_animClip._targetMeshGroup._rootRenderUnit != null)
					{
						_animClip._targetMeshGroup._rootRenderUnit.CalculateRenverVertsWorldPosNoMod(apRenderUnit.CALCULATE_NO_MOD_POS_REQUEST.OnlyAdaptMatrix_WithChild);
					}
				}
		...
		private apHotKey.HotKeyResult OnHotKeyEvent_RenameControlParam(object paramObject)
		{
			...
			//변경 v1.4.5
			apControlParam controlParam = paramObject as apControlParam;
			bool isProcessed = false;
			if(controlParam != null)
			{
				_loadKey_RenameControlParam = apDialog_Rename.ShowDialog(	_editor,
																			controlParam,
																			null,
																			controlParam._keyName,
																			OnRenameResult_ControlParam);
				isProcessed = true;
			}
		...
		//컨트롤 파라미터 이름을 다이얼로그에서 바꾸기
		private object _loadKey_RenameControlParam = null;
		public void OnRenameResult_ControlParam(bool isSuccess, object loadKey, object targetObject, apEditorHierarchyUnit targetHierarchyUnit, string name)
		{
			if(!isSuccess
				|| loadKey == null
				|| _loadKey_RenameControlParam != loadKey
				|| targetObject == null
				|| string.IsNullOrEmpty(name)
				|| _editor == null
				|| _editor._portrait == null)
			{
				_loadKey_RenameControlParam = null;
				return;
			}
			_loadKey_RenameControlParam = null;

			//컨트롤 파라미터의 이름은 고유해야한다.
			if(targetObject is apControlParam)
			{
				...
				if (controlParam != null)
				{
					//다음 코드는 apSelection_UI의 DrawParam 내의 코드와 같다.
					...
					else if (Editor.ParamControl.FindParam(name) != null)
					{
						//이미 사용중인 이름이다.
						EditorUtility.DisplayDialog(Editor.GetText(TEXT.ControlParamNameError_Title),
					...
					else
					{
						...
						Editor.Controller.ChangeParamName(controlParam, name);
					}
				}
			}
			Editor.RefreshControllerAndHierarchy(false);
