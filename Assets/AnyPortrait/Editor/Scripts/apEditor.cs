...
//v1.4.8 스파이크 체크 로직 (Unity 2023 전용)
		...
		void OnGUI()
		{
			...
			try
			{
				...
				if (_isFirstOnGUI)
				{
					...
					if (apVersion.I.IsDemo)//시작 다이얼로그 제한
					{
					...
					_gizmos.IsDrawFlag)
				{
					...
					//기즈모 그리기 :
					//[v1.4.5 버그 수정] Repaint에서만 그리기가 되어야 한다.
					if(Event.current.type == EventType.Repaint)//<<이 코드가 추가됨
					{
						...
						Event.current.type == EventType.KeyUp)
					{
						if (GUIUtility.keyboardControl == 0)
						{
							...
							if (eventResult != apHotKey.EVENT_RESULT.None)//변경
							{
								...
								//[v1.4.5] 만약 키 입력 직후 윈도우 포커스를 잃었다면
								//Up 이벤트가 무시되어 키 입력을 한번 더 해야하는 버그 발생
								if(EditorWindow.focusedWindow != this)
								{
									//Debug.LogError("단축키 입력 후 포커스를 잃었다");

									//자연스럽게 처리되어야 하는 Up 이벤트가 제대로 처리가 되지 않았을 것
									HotKey.SetKeyReleasedForce();
								}
								//else
								//{
								//	Debug.Log("단축키 입력 후에도 포커스 유지");
								//}
							}
						}
					}
		...
		private void GUI_Top(int width, int height)
		{
			...
			if (IsDelayedGUIVisible(DELAYED_UI_TYPE.Top_UI__Overall))//"Top UI - Overall"
			{
				...
				if (isGUITab_Capture)
				{
					...
					{
						if (apVersion.I.IsDemo)//재질 라이브러리
						{
							//추가 : 데모 버전일 때에는 재질 라이브러리가 제한된다.
							EditorUtility.DisplayDialog(
											GetText(TEXT.DemoLimitation_Title),
											GetText(TEXT.DemoLimitation_Body),
											GetText(TEXT.Okay));
		...
		private int GUI_LeftUpper(int width)
		{
			...
			if (_portrait == null)
			{
				...
				{
					if (apVersion.I.IsDemo)//백업 로드 제한
					{
		...
		private void GUI_Center(int width, int height)
		{
			...
			if (_isShowCaptureFrame && _portrait != null && Select.SelectionType == apSelection.SELECTION_TYPE.Overall)
			{
				...
				//변경 v1.4.6 : Position Y의 좌표 반전
				Vector2 framePos_Center = new Vector2(	_captureFrame_PosX + apGL.WindowSizeHalf.x,
														(-1 * _captureFrame_PosY) + apGL.WindowSizeHalf.y);
			...
			////테스트 : 툴팁 (GUI.tooltip을 사용하는 방법만 익혀보았다.)
			//if(!string.IsNullOrEmpty(GUI.tooltip))
			//{
			//	apGL.DrawTextGL(GUI.tooltip, new Vector2(GUI_STAT_MARGIN, guiLabelY), 400, Color.yellow);
			//	guiLabelY += 15;
			//}
