...
namespace AnyPortrait
{
	...
	public class apDialog_ControlParamPreset : EditorWindow
	{
		...
		void OnGUI()
		{
			...
			GUILayout.Space(5);
			apEditorUtil.GUI_DelimeterBoxH(width - 2);
			GUILayout.Space(5);
			...
			if (_selectedUnit != null)
			{
				...
				//오른쪽 영역				
				//Label
				//SnapSize (Float / Vector인 경우)
				//Index 옵션 (Int인 경우)
				//삭제 버튼 (Reserved가 아닌 경우)
				...
				if(_selectedUnit._valueType == apControlParam.TYPE.Float
					|| _selectedUnit._valueType == apControlParam.TYPE.Vector2)
				{
					GUILayout.Space(5);
				...
				if(_selectedUnit._valueType == apControlParam.TYPE.Int)
				{
					GUILayout.Space(5);
					EditorGUILayout.Toggle(_editor.GetUIWord(UIWORD.IndexType), _selectedUnit._isNonInterpolatedIndex, GUILayout.Width(bottomInfoWidth));
				}
