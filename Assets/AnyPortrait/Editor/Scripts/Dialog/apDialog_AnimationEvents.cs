...
namespace AnyPortrait
{
	public class apDialog_AnimationEvents : EditorWindow
	{
		...
		//[1.4.5] 오픈시 카테고리 리스트 > 배열을 만들자
		private string[] _presetCategoryNames = null;
		private string _selectedPresetCategoryName = "";
		private int _iSelectedPresetCategory = 0;

		//현재 카테고리의 프리셋들
		private List<apAnimEventPresetUnit> _eventPresets = null;

		private const string PREF_SELECTED_PRESET_NAME = "AnyPortrait_LastSelectedAnimEventPresetName";
		private const string COMMON_PRESET_CATEGORY = "Everything";

		//프리셋 설정 화면에 들어갈 카테고리 리스트 (구성이 조금 다르다)
		private const string NONE_CATEGORY = "<None>";
		...
		public static void ShowDialog(apEditor editor, apPortrait portrait, apAnimClip animClip)
		{
			...
			{
				int width = 850;//라인이 두개가 되면서 조금 더 커졌다.
				int height = 700;
				s_window = curTool;
		...
		/// <summary>
		/// 만약 애니메이션 클립이 선택되었을 때, 윈도우창이 열려져 있었다면
		/// 내용을 한번 갱신해볼 수 있다.
		/// </summary>
		public static void OnAnimClipChanged()
		{
			try
			{
				if (s_window != null)
		...
		public void Init(apEditor editor, apPortrait portrait, apAnimClip animClip)
		{
			...
			//이벤트 프리셋들도 검사를 하고, UI용 배열 변수를 체크한다.
			CheckAndRefreshPresets();
		...
		//-------------------------------------------------------------------------
		/// <summary>
		/// 프리셋 추가/삭제시 이 함수를 호출해서 프리셋 리스트를 다시 받아와야 한다.
		/// </summary>
		private void CheckAndRefreshPresets()
		{	
			//저장된 이름에서 선택했었던 프리셋을 가져온다.
			_selectedPresetCategoryName = EditorPrefs.GetString(PREF_SELECTED_PRESET_NAME, "");
			_iSelectedPresetCategory = 0;

			List<string> presetCategories = _editor.AnimEventPreset.Categories;
			int nPresetCats = presetCategories != null ? presetCategories.Count : 0;

			List<string> categoryNames = new List<string>();//실제로 카테고리의 이름은 "Everything" 때문에 하나 더 많다.
			if(_propNameList_PresetCategoryInSettings == null) { _propNameList_PresetCategoryInSettings = new List<string>(); }
			_propNameList_PresetCategoryInSettings.Clear();

			//기본 이름 추가
			categoryNames.Add(COMMON_PRESET_CATEGORY);

			//<None> 추가
			_propNameList_PresetCategoryInSettings.Add(NONE_CATEGORY);

			//프리셋을 하나씩 추가하자
			for (int i = 0; i < nPresetCats; i++)
			{
				string curCatName = presetCategories[i];
				if (!categoryNames.Contains(curCatName))
				{
					categoryNames.Add(curCatName);
					_propNameList_PresetCategoryInSettings.Add(curCatName);//Prop에도 추가
				}
			}

			//Prop에 Add Category도 추가
			_propNameList_PresetCategoryInSettings.Add(ADD_CATEGORY);


			//배열로 변환한다.
			_presetCategoryNames = categoryNames.ToArray();

			//속성으로서의 카테고리 이름들도 설정
			_propNames_PresetCategoryInSettings = _propNameList_PresetCategoryInSettings.ToArray();

			//현재 선택된 카테고리 이름이 포함되어 있는지 확인
			if(categoryNames.Contains(_selectedPresetCategoryName))
			{
				//포함되어 있다면
				_iSelectedPresetCategory = categoryNames.IndexOf(_selectedPresetCategoryName);
			}
			else
			{
				//없다면 Default로 
				_selectedPresetCategoryName = COMMON_PRESET_CATEGORY;
				_iSelectedPresetCategory = 0;

				//변경되었다면 저장을 하자
				EditorPrefs.SetString(PREF_SELECTED_PRESET_NAME, _selectedPresetCategoryName);
			}

			if(_eventPresets == null)
			{
				_eventPresets = new List<apAnimEventPresetUnit>();
			}
			_eventPresets.Clear();
			...
			List<apAnimEventPresetUnit> srcPresets = null;

			if(_iSelectedPresetCategory == 0)
			{
				//기본 인덱스라면 전체 조회
				srcPresets = _editor.AnimEventPreset.Presets;
			}
			else
			{
				//그 외라면 이름을 바탕으로 조회를 한다.
				srcPresets = _editor.AnimEventPreset.GetUnitsByCategory(_selectedPresetCategoryName);
			}

			int nSrcPresets = srcPresets != null ? srcPresets.Count : 0;

			//프리셋 리스트에 넣자
			for (int i = 0; i < nSrcPresets; i++)
			{
				_eventPresets.Add(srcPresets[i]);
			}

			//프리셋을 알파벳 순으로 정렬한다.
			_eventPresets.Sort(delegate(apAnimEventPresetUnit a, apAnimEventPresetUnit b)
			{
			...
			{
				if(!_eventPresets.Contains(_selectedPreset))
				{
					_selectedPreset = null;
				}
			}
		...
		private void ChangePresetCategory(int iNextCategory)
		{
			_iSelectedPresetCategory = iNextCategory;
				
			int nCategories = _presetCategoryNames != null ? _presetCategoryNames.Length : 0;
			if (nCategories == 0)
			{
				_iSelectedPresetCategory = 0;
				_selectedPresetCategoryName = "";
			}
			else
			{
				if (_iSelectedPresetCategory < 0)
				...
				else if (_iSelectedPresetCategory >= nCategories)
				{
					_iSelectedPresetCategory = nCategories - 1;
				}

				_selectedPresetCategoryName = _presetCategoryNames[_iSelectedPresetCategory];
			}
		...
		/// <summary>
		/// 입력된 프리셋이 리스트에 보여지고 선택되도록 카테고리를 자동으로 변경한다.
		/// </summary>
		/// <param name="targetPreset"></param>
		private void AutoChangeCategoryForTargetPreset(apAnimEventPresetUnit targetPreset)
		{
			...
			string curCategory = targetPreset._customCategory;

			int nextCatIndex = 0;

			//카테고리가 없다면
			int nCategories = _presetCategoryNames != null ? _presetCategoryNames.Length : 0;
			if (nCategories <= 1 || string.IsNullOrEmpty(curCategory))
			{
				//카테고리 이름이 유효하지 않거나 1개 또는 0개의 카테고리만 있다면
				//0번째 카테고리 (Everything)를 선택한다.
				nextCatIndex = 0;
			}
			else
			{
				//이 프리셋의 이름이 포함된 리스트를 찾자
				//0은 전체이므로, 1부터 찾는다.
				int iSpecificCategory = -1;

				for (int i = 1; i < nCategories; i++)
				{
					if (string.Equals(_presetCategoryNames[i], curCategory))
					{
						//해당 카테고리를 찾았다.
						iSpecificCategory = i;
						break;
				...
				{
					//적절한 카테고리를 찾았다.
					nextCatIndex = iSpecificCategory;
				}
			}

			if(_iSelectedPresetCategory == 0)
			{
				//만약 "전체 리스트(Everything)"를 선택하고 있었다면
				//그냥 그거 유지
				nextCatIndex = 0;
			}
			//인덱스를 변경하자

			ChangePresetCategory(nextCatIndex);

			//현재 리스트에 프리셋이 있다면 선택을 다시 하자
			if(_eventPresets != null
				&& _eventPresets.Contains(targetPreset))
			{
				_selectedPreset = targetPreset;
			}
		}
		...
		void OnGUI()
		{
			...
			//적용할 것
			if(_editor.Select.AnimClip != _animClip)
			{
				//애니메이션 변경 사항을 저장하자
				_animClip = _editor.Select.AnimClip;
				...
				//일부 UI 정보도 리셋
				_guiContent_AnimClipName = null;//타이틀 정보
			}
		...
		private void GUI_Left(int width, int height, int posX, int posY, bool isGUIEvent)
		{
			...
			GUI.Box(new Rect(posX, posY - 1, width + 1, height_1_List + 2), "");
			...
			GUI.Box(new Rect(posX, posY + height_1_List + height_2_ItemButtons + height_3_Properties + 98, width + 1, height_4_ParameterList + 2), "");
			GUI.backgroundColor = prevColor;
		...
		private void GUI_Right(int width, int height, int posX, int posY, bool isGUIEvent)
		{
			...
			int height_0_Title = 25;
			int height_0_2_Category = 22;
			int height_2_1_AdaptButton = 26;
			int height_2_2_AddButton = 22;
			int height_3_PresetProperties = 172;
			int height_4_PresetParams = 80;
			int height_5_RemoveButton = 20;
			int height_1_PresetList = height 
				- (height_0_Title + height_3_PresetProperties + height_2_1_AdaptButton + height_2_2_AddButton + height_4_PresetParams + height_5_RemoveButton + height_0_2_Category + 65);
			...
			EditorGUILayout.BeginVertical(GUILayout.Width(width), GUILayout.Height(height_0_2_Category));

			int iNextCategory = EditorGUILayout.Popup(_iSelectedPresetCategory, _presetCategoryNames);
			if(iNextCategory != _iSelectedPresetCategory)
			{
				//프리셋 카테고리를 교체하자
				ChangePresetCategory(iNextCategory);
				apEditorUtil.ReleaseGUIFocus();//포커스를 날린다.
			}
			...
			int nPresets = _eventPresets != null ? _eventPresets.Count : 0;

			GUI.backgroundColor = new Color(0.9f, 0.9f, 0.9f);
			GUI.Box(new Rect(posX + 3, posY + height_0_Title + 6 + height_0_2_Category, width + 1, height_1_PresetList + 2), "");
			GUI.backgroundColor = prevColor;
			...
			// 3. 프리셋 적용 버튼
			//-------------------------------
			//v1.4.5 : 변경점 : 선택된 이벤트가 없다면 바로 생성

			//버튼이 두개로 바뀌었다.
			//- 현재 선택된 이벤트로 적용
			//- 현재 프레임에 추가
			if(apEditorUtil.ToggledButton_2Side(	_editor.GetText(TEXT.ApplySelectedPreset),
													false,
													_selectedPreset != null && _curSelectedEvent != null,
													width_Prop, height_2_1_AdaptButton))
			{	
				if(_curSelectedEvent != null)
				{
					//현재 선택된 이벤트가 있다면
			...
			//현재 프레임에 추가하기 (이벤트가 선택되지 않아도 됨)
			if(apEditorUtil.ToggledButton_2Side(	_editor.GetText(TEXT.AddEventByPreset),
													false,
													_selectedPreset != null 
													, width_Prop, height_2_2_AddButton))
			{
				//[v1.4.5 추가]
				//현재 선택된 이벤트에 상관없이
				//추가하고 적용하기
				...
				bool result = EditorUtility.DisplayDialog(
									_editor.GetText(TEXT.DLG_ApplyAnimEventPreset_Ttitle),
									_editor.GetTextFormat(TEXT.DLG_ApplyAnimEventPresetToCurrentFrame_Body, _selectedPreset._eventName, curFrame),
									_editor.GetText(TEXT.DLG_Add),
									_editor.GetText(TEXT.Cancel));

				if(result)
				{
					//현재 선택된 프리셋을 적용한 이벤트를 만들자
					apEditorUtil.SetRecord_Portrait(apUndoGroupData.ACTION.Anim_EventChanged,
																	_editor,
																	_animClip._portrait,
																	//null, 
			...
			//현재 카테고리 인덱스
			string curProp_CatName = "";
			int curProp_CatIndex = 0;//이건 Prop 리스트 상의 인덱스이다.
			...
			if(_selectedPreset != null)
			{
				...
				curProp_CatName = _selectedPreset._customCategory;
				int nCatList = _propNameList_PresetCategoryInSettings != null ? _propNameList_PresetCategoryInSettings.Count : 0;
				if(nCatList > 0 && !string.IsNullOrEmpty(curProp_CatName))
				{
					curProp_CatIndex = _propNameList_PresetCategoryInSettings.IndexOf(curProp_CatName);
				}
				else
				{
					curProp_CatIndex = 0;
			...
			//카테고리 리스트는
			// - Everything을 제외한
			// - <None>, (기존 카테고리들), <새로 추가하기>
			GUILayout.Space(4);
			EditorGUILayout.LabelField(_editor.GetText(TEXT.DLG_Category));
			int nextCatIndex = EditorGUILayout.Popup(curProp_CatIndex, _propNames_PresetCategoryInSettings);


			if(EditorGUI.EndChangeCheck())
			{
				//인덱스의 변경이 있다면 (단순 설정 변경이 아닌)
				if(_selectedPreset != null)
				{
					if (nextCatIndex != curProp_CatIndex)
					{
						int nCatList = _propNameList_PresetCategoryInSettings != null ? _propNameList_PresetCategoryInSettings.Count : 0;

						//0. None : 카테고리를 삭제한다.
						//1 ~ n-2. 카테고리 : 특정 카테고리로 변경한다.
						//n-1. 카테고리 추가 : 카테고리를 새로 추가한다.
						if (nextCatIndex == 0)
						{
							//None : 카테고리를 삭제한다.
							//카테고리 리스트가 전체 바뀔 수 있다.
							_selectedPreset._customCategory = ""; // 삭제
							_editor.AnimEventPreset.RefreshCategory();//프리셋 클래스에서 갱신
							_editor.AnimEventPreset.Save();

							//이 다이얼로그에서의 카테고리 리스트 갱신
							//이때 이 함수를 호출하면 _selectedPreset가 null이 될 수 있으므로,
							//다른 변수를 적용하고, 카테고리 자동 선택 및 복원을 할 수 있도록 하자
							apAnimEventPresetUnit prevSelected = _selectedPreset;

							CheckAndRefreshPresets();

							//자동으로 카테고리 선택 및 선택 복원
							AutoChangeCategoryForTargetPreset(prevSelected);
						}
						else if (nextCatIndex < nCatList - 1)
						{
							//해당 카테고리 명을 적용한다.
							_selectedPreset._customCategory = _propNameList_PresetCategoryInSettings[nextCatIndex];

							_editor.AnimEventPreset.RefreshCategory();//프리셋 클래스에서 갱신
							_editor.AnimEventPreset.Save();

							//프리셋을 변경하면서 카테고리 리스트를 갱신하고, 선택도 다시 복원
							apAnimEventPresetUnit prevSelected = _selectedPreset;
						...
						else if (nextCatIndex == nCatList - 1)
						{
							//< New Category >
							//이건 새로운 창을 띄워야 한다.
							_loadKey_NewCategoryName = apDialog_AnimEventNewCategory.ShowDialog(_editor, _selectedPreset, OnNewCategoryNameAdded);
			...
			//----------------------------------------------------
			GUI.backgroundColor = new Color(0.9f, 0.9f, 0.9f);
			GUI.Box(new Rect(	posX + 3,
								posY + height_0_Title + height_1_PresetList + height_2_1_AdaptButton + height_2_2_AddButton + height_3_PresetProperties + 30 + height_0_2_Category,
								width + 1,
								height_4_PresetParams + 2), "");
			GUI.backgroundColor = prevColor;
		...
		private object _loadKey_NewCategoryName = null;
		private void OnNewCategoryNameAdded(bool isSuccess, object loadKey, object targetObject, string newName)
		{
			//카테고리를 변경한 경우
			if(!isSuccess
				|| _loadKey_NewCategoryName != loadKey
				|| _loadKey_NewCategoryName == null
				|| loadKey == null
				|| _selectedPreset == null
				|| _selectedPreset != targetObject)
			{
				//조건이 맞지 않다.
				_loadKey_NewCategoryName = null;
				return;
			...
			if(string.Equals(newName, COMMON_PRESET_CATEGORY))
			{
				newName = "";
			}
			...
			newName = newName.Replace("\t", "");
			newName = newName.Replace("-", " ");//이건 빈칸

			//카테고리를 지정하고
			_selectedPreset._customCategory = newName;
