	...
	public class apPSDGL
	{
			...
			try
			{
				...
				else
				{
					...
					for (int i = 0; i < nRenderVerts; i++)
					{
						...
						//Mod 적용 안된걸로 + Root Matrix 역연산

						//이전
						//rVert._pos_GL = World2GL(rootMatrix.InvMulPoint2(rVert._pos_World_NoMod));

						//변경 v1.4.4 : Ref를 이용
						rootMatrix.InvMulPoint2(ref tmpPos, ref rVert._pos_World_NoMod);
						rVert._pos_GL = World2GL(tmpPos);
					}
				}
																...
																apMatrix rootMatrix = null)
		{
			...
			try
			{
				...
				Vector2 tmpPos_0 = Vector2.zero;
				Vector2 tmpPos_1 = Vector2.zero;
				Vector2 tmpPos_2 = Vector2.zero;
				...
				for (int iPass = 0; iPass < 2; iPass++)
				{
					...
					for (int i = 0; i < mesh._indexBuffer.Count; i += 3)
					{
						...
						else
						{
							...
							//변경 v1.4.4							
							rootMatrix.InvMulPoint2(ref tmpPos_0, ref rVert0._pos_World_NoMod);
							rootMatrix.InvMulPoint2(ref tmpPos_1, ref rVert1._pos_World_NoMod);
							rootMatrix.InvMulPoint2(ref tmpPos_2, ref rVert2._pos_World_NoMod);
