...
namespace AnyPortrait
{
	...
	public class apAnimEventPreset
	{
		...
		//카테고리들을 정리하자 (프리셋 변경시 항상 호출)
		private List<string> _categories = null;
		private Dictionary<string, List<apAnimEventPresetUnit>> _category2Units = null;
		...
		private const string DEFAULT_CATEGORY = "Everything";//이건 카테고리 이름으로 적용되지 않는다.
		...
		public void Clear()
		{
			...
			if(_category2Units == null) { _category2Units = new Dictionary<string, List<apAnimEventPresetUnit>>(); }
			_category2Units.Clear();
		}
		...
		//[v1.4.5] 카테고리를 갱신한다.
		/// <summary>
		/// 카테고리를 갱신한다. 유닛 추가/삭제시에는 자동으로 호출되며,
		/// 설정상에서 유닛 프리셋의 카테고리 이름 변경시에는 이 함수를 꼭 호출하자
		/// </summary>
		public void RefreshCategory()
		{
			if(_categories == null) { _categories = null; }
			_categories.Clear();
			...
			if(nUnits > 0)
			{
				...
				for (int i = 0; i < nUnits; i++)
				{
					...
					//카테고리가 지정된 경우엔 추가
					if(string.IsNullOrEmpty(curUnit._customCategory))
					{
						//없다면 패스
						continue;
					}

					if(string.Equals(curUnit._customCategory, DEFAULT_CATEGORY))
					{
						//특정 카테고리 이름("Everything")은 적용될 수 없다.
						continue;
					}
					//일부 특수 문자는 제외한다.
					curUnit._customCategory = curUnit._customCategory.Replace("\n", "");
					curUnit._customCategory = curUnit._customCategory.Replace("\r", "");
					curUnit._customCategory = curUnit._customCategory.Replace("\t", "");
					curUnit._customCategory = curUnit._customCategory.Replace("-", " ");//이건 띄어쓰기로 변경
					...
					string curCategory = curUnit._customCategory;
					
					if(!_categories.Contains(curCategory))
					{
						//카테고리 추가
						_categories.Add(curCategory);
					}

					if(!_category2Units.ContainsKey(curCategory))
					{
						_category2Units.Add(curCategory, new List<apAnimEventPresetUnit>());
					}
					
					_category2Units[curCategory].Add(curUnit);
				}
			}
			...
			{
				_categories.Sort(delegate(string a, string b)
				{
		...
		/// <summary>
		/// 카테고리별 프리셋 유닛 리스트를 가져온다. null이나 빈칸 입력시에는 전체 리스트가 리턴된다.
		/// 에러 또는 데이터 미비로 null 리스트가 리턴될 수 있으니 꼭 확인할 것
		/// </summary>
		public List<apAnimEventPresetUnit> GetUnitsByCategory(string category)
		{
			...
			List<apAnimEventPresetUnit> targetList = null;
			_category2Units.TryGetValue(category, out targetList);

			return targetList;//Null일 수 있다.
		}
	...
	public class apAnimEventPresetUnit
	{
		...
		public void EncodeToText(StreamWriter sw)
		{
			...
			//[v1.4.5 추가]
			if(string.IsNullOrEmpty(_customCategory))
			{
				_customCategory = "";
			}
			_customCategory = _customCategory.Replace("\n", "");
			_customCategory = _customCategory.Replace("\r", "");
			_customCategory = _customCategory.Replace("\t", "");
			if(_customCategory.Length == 0)
			{
				//빈값의 고정값은 "--"이다.
				sw.WriteLine("CTG:--");
			}
			...
			{
				sw.WriteLine("CTG:" + _customCategory);
			}
		...
		public bool DecodeFromText(List<string> strData)
		{
			...
			for (int i = 0; i < nStrData; i++)
			{
				...
				if(isParse_Basic)
				{
					...
					else if(string.Equals(strKey, "CTG"))
					{
						//[v1.4.5 추가 ] 카테고리
						_customCategory = strValue;
						if(string.Equals(_customCategory, "--"))
						{
							_customCategory = "";//--는 빈칸과 같다.
						}
					}
