...
namespace AnyPortrait
{
	...
	public enum TEXT : int
	{
		...
		DLG_ApplyAnimEventPresetToCurrentFrame_Body = 819,
		AddEventByPreset = 820,
		DLG_SaveScenesAfterBake = 821,
		Setting_EditorUI = 822,
		Setting_ColorSelectedTimeline = 823,
		Setting_TimelineDefaultColor = 824,
		DLG_CaptureOverMaxSize_Title = 825,
	...
	public class apLocalization
	{
		...
		private int _nTextTypes = 0;
		private int _nWordTypes = 0;
		private string[] _textArr = null;
		private string[] _uiWordArr = null;
		...
		public void SetTextAsset(apEditor.LANGUAGE language, TextAsset textAsset_Dialog, TextAsset textAsset_UI)
		{
			...
			//배열로도 만들자 [v1.4.5]
			_nTextTypes = apEditorUtil.GetEnumCount(typeof(TEXT));
			_nWordTypes = apEditorUtil.GetEnumCount(typeof(UIWORD));

			_textArr = new string[_nTextTypes];
			_uiWordArr = new string[_nWordTypes];
			...
			for (int i = 1; i < strParseLines.Length; i++)
			{
				...
				{	
					...
					//[v1.4.5]
					_textArr[(int)textType] = strCurText;
				}
				catch (Exception)
				{
			...
			for (int i = 1; i < strParseLines.Length; i++)
			{
				...
				try
				{
					...
					//변경된 코드
					 //[v1.4.5]
					_uiWordArr[(int)uiWordType] = strCurText;
				}
				catch (Exception)
				{
					Debug.LogError("Parsing 실패 (" + i + " : " + strCurParseLine + ")");
		...
		public string GetUIWord(UIWORD uiWordType)
		{
			...
			//변경 v1.4.5
			return _uiWordArr[(int)uiWordType];
		}
