...
namespace AnyPortrait
{
	...
	public static class apTimelineGL
	{
		...
		public static void DrawAndUpdateSelectArea(bool isRepaintEvent)
		{
			...
			if (isRepaintEvent)//[v1.4.5] Repaint 이벤트일때 그리기
			{
		...
		/// <returns></returns>
		public static bool DrawPlayBar(int frame, bool isRepaintEvent)
		{
		...
		public static void DrawKeySummry(List<apAnimCommonKeyframe> commonKeyframes, float posY, bool isRepaintEvent)
		{
			float posX = 0.0f;
			...
			int nCommonKeyframes = commonKeyframes != null ? commonKeyframes.Count : 0;

			if (nCommonKeyframes > 0)
			{
				//선택 방식
				//이전 : 하나씩 현재 상태, 클릭 상태 확인하여 선택 처리.

				if (isRepaintEvent)//[v1.4.5] Repaint에서만
				{
					//Batch하여 렌더링을 하자
				...
				for (int i = 0; i < nCommonKeyframes; i++)
				{
			...
			//드래그시의 키프레임도 보여주자 
			if (isRepaintEvent)//[v1.4.5] Repaint 이벤트로 제한
			{

				int nMoveCommonKeys = _moveCommonKeyframeList != null ? _moveCommonKeyframeList.Count : 0;
				if (nMoveCommonKeys > 0)
				{
					MoveCommonKeyframe moveCopyFrame = null;
					for (int i = 0; i < nMoveCommonKeys; i++)
					{
						moveCopyFrame = _moveCommonKeyframeList[i];
		...
		public static void DrawEventMarkers(List<apAnimEvent> animEvents, float posY, bool isRepaintEvent)
		{
			//DrawTextureAtlas
			...
			for (int i = 0; i < nAnimEvents; i++)
			{
				...
				//TODO : 나중에 여기서 마우스 커서 롤오버 체크 (연속 범위)
											...
											)
		{
			...
			if (isCurveEdit 
				&& isSelectedTimelineLayer
				&& isRepaintEvent//[v1.4.5] Repaint 이벤트에서만 그리도록
				)
			{
			...
			for (int i = 0; i < keyFrames.Count; i++)
			{
				...
				DrawSingleKeyframe(	curKeyFrame,
									isAvailable,
									sizeW,
									sizeH,
									isDrawY,
			...
			// < 렌더링 >
			if (isRepaintEvent)//v1.4.5 : 렌더링은 Repaint 이벤트에서만
			{
											...
											)
		{
			...
			if (isDrawX && isDrawY)
			{
				if (isRepaintEvent)//[v1.4.5] 렌더링 요청은 Repaint 이벤트에서만
				{
					if (!isDummyFrame)
					{
