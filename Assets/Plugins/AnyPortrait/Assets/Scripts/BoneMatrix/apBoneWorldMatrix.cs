...
namespace AnyPortrait
{
	...
	public class apBoneWorldMatrix
	{
		...
		//변경 v1.4.4 : Ref를 이용하여 성능 향상
		public void MulPoint2(ref Vector2 dstPos, ref Vector2 srcPoint)
		{
			if (_scaleMethod == apPortrait.ROOT_BONE_SCALE_METHOD.Default)
			{
				_mtx_Def.MulPoint2(ref dstPos, ref srcPoint);
			}
			else
			{
				_mtx_Skew.MulPoint2(ref dstPos, ref srcPoint);
			}
		}
		...
		//변경 v1.4.4 : Ref를 이용하여 성능 향상
		public void InvMulPoint2(ref Vector2 dstPos, ref Vector2 srcPoint)
		{
			if (_scaleMethod == apPortrait.ROOT_BONE_SCALE_METHOD.Default)
			{
				_mtx_Def.InvMulPoint2(ref dstPos, ref srcPoint);
			}
			else
			{
				_mtx_Skew.InvMulPoint2(ref dstPos, ref srcPoint);
			}
		}
