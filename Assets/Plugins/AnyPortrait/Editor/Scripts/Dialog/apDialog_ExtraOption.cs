	...
	public class apDialog_ExtraOption : EditorWindow
	{
							...
							apAnimClip animClip)
		{
			...
			if (isAnimEdit)
			{
				...
				{
					//return a._linkedModifier = modifier;//완전한 버그..
					return a._linkedModifier == modifier;//[v1.4.6] 수정
				});
			}
			else
			{
		...
		private void OnGUI()
		{
			...
			if (_tab == TAB.Depth)
			{
				...
				bool isAvailableDepth = isDepth_Available && isExtraProp_Enabled && isExtraProp_Sync;

				//이전
				//apEditorUtil.ToggledButton_2Side_Sync...

				

				//변경 v1.4.6 : 무조건 클릭 가능
				if (apEditorUtil.ToggledButton_2Side_Sync_ForceClickEvent(	_editor.GetText(TEXT.ExtraOpt_DepthOptOn), 
														...
														width, 25))
				{
					...
					//[v1.4.6]
					//만약 Available이 false인데 억지로 클릭을 했을 경우
					//Extra Option을 사용 가능하기만 하다면 (On/Off는 무관)
					//동기화를 먼저 해서 시도할 수 있다.
					bool isUndoPerformed = false;
					bool isDepthExtraChangable = false;

					if (!isAvailableDepth)
					{
						//비록 현재 ExtraOption이 꺼져 있지만
						//ExtraOption 자체를 켤 수 있다면

						if (isExtraProp_Available)
						{
							//ExtraOption 자체를 여기서 빠르게 켤 수 있다.
							apEditorUtil.SetRecord_Modifier(	apUndoGroupData.ACTION.Modifier_ExtraOptionChanged,
																_editor,
																_modifier,
																false,
							...
							SyncValue_Bool(SYNC_VAR_TYPE_BOOL.ExtraOption, true);
							isUndoPerformed = true;//여기서 Undo를 수행했다면
							isDepthExtraChangable = true;//Depth 옵션을 변경할 수 있다.
						}
					}
					else
					{
					...
					if(isDepthExtraChangable)
					{
						if(!isUndoPerformed)
						{
							//Undo를 실행하지 않았다면 실행하자
							apEditorUtil.SetRecord_Modifier(	apUndoGroupData.ACTION.Modifier_ExtraOptionChanged, 
																_editor, 
																_modifier, 
																false,
			...
			else
			{
				...
				bool isAvailableTexture = isTextureChanged_Available && isExtraProp_Enabled && isExtraProp_Sync;

				if (apEditorUtil.ToggledButton_2Side_Sync_ForceClickEvent(	_editor.GetText(TEXT.ExtraOpt_ImageOptOn), 
																			...
																			width, 25))
				{
					...
					//동기화를 먼저 해서 시도할 수 있다.
					bool isUndoPerformed = false;
					bool isTextureExtraChangable = false;

					if (!isAvailableTexture)
					{
						//비록 현재 ExtraOption이 꺼져 있지만
						//ExtraOption 자체를 켤 수 있다면
						...
						if (isExtraProp_Available)
						{
							...
							SyncValue_Bool(SYNC_VAR_TYPE_BOOL.ExtraOption, true);
							isUndoPerformed = true;//여기서 Undo를 수행했다면
							isTextureExtraChangable = true;//Texture 옵션을 변경할 수 있다.
						}
					}
					else
					{
