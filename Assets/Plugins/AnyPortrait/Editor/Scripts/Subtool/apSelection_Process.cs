	...
	public partial class apSelection
	{
			...
			if (rootUnit != null)
			{
				...
				if(_portrait._objectOrders != null)
				{
					...
					{	
						...
						{
							curOrderSet = animClipOrderSet[i];
							if(curOrderSet == null)
							{
								continue;
							}
							if(curOrderSet._linked_AnimClip == null)
							{
								continue;
							}
							animClip = curOrderSet._linked_AnimClip;
							if (_rootUnit._childMeshGroup == animClip._targetMeshGroup)
							{
								_rootUnitAnimClips.Add(animClip);//<<연동되는 AnimClip이다.
							}
				...
				if (_rootUnit._childMeshGroup != null)
				{
					...
					//추가 v1.4.5 : 모디파이어의 Enable을 모두 켠다.
					_rootUnit._childMeshGroup._modifierStack.EnableAllModifierAndPSGs();
					...
					if(_rootUnit._childMeshGroup._rootRenderUnit != null)
					{
						_rootUnit._childMeshGroup._rootRenderUnit.CalculateRenverVertsWorldPosNoMod(apRenderUnit.CALCULATE_NO_MOD_POS_REQUEST.OnlyAdaptMatrix_WithChild);
					}
		...
		public void SelectMeshGroup(apMeshGroup meshGroup)
		{
			...
			if (isChanged)
			{
				...
				//추가 v1.4.5 : 모디파이어의 Enable을 모두 켠다.
				_meshGroup._modifierStack.EnableAllModifierAndPSGs();
			...
			//다 계산한 World Matrix 중 No-Mod에 의한 위치값이 업데이트 순서 문제로 RenderVertex에 반영되지 않는 문제
			//[계산한 World Matrix 적용 + 모든 자식 갱신] 옵션
			_meshGroup._rootRenderUnit.CalculateRenverVertsWorldPosNoMod(apRenderUnit.CALCULATE_NO_MOD_POS_REQUEST.OnlyAdaptMatrix_WithChild);
		...
		public void SelectAnimClip(apAnimClip animClip)
		{
			...
			if (_animClip != animClip)
			{
				...
				if (_animClip._targetMeshGroup != null)
				{
					...
					//추가 v1.4.5 : 모디파이어의 Enable을 모두 켠다.
					_animClip._targetMeshGroup._modifierStack.EnableAllModifierAndPSGs();
			...
			if (isChanged && _animClip != null)
			{
				//타임라인을 자동으로 선택해주자
				//이전 : 무조건 첫 타임라인을 선택한다.
				//변경 [v1.4.5] : 이전에 선택한 타임라인 타입과 동일한걸 먼저 선택한다.
				int nTimelines = _animClip._timelines != null ? _animClip._timelines.Count : 0;
				if (nTimelines > 0)
				{
					bool isTimelineSelected = false;
					if(_lastAnimTimelineType != LAST_ANIM_TIMELINE_TYPE.Unknown)
					{
						//이전에 선택한 타입이 있을 때, 동일한 타입의 타임라인을 찾자
						apAnimTimeline targetTimeline = _animClip._timelines.Find(delegate(apAnimTimeline a)
						{
							switch (_lastAnimTimelineType)
							{
								case LAST_ANIM_TIMELINE_TYPE.ControlParam:
									return a._linkType == apAnimClip.LINK_TYPE.ControlParam;

								case LAST_ANIM_TIMELINE_TYPE.ModTF:
									{
										if(a._linkType == apAnimClip.LINK_TYPE.AnimatedModifier && a._linkedModifier != null)
										{
											return (a._linkedModifier.ModifierType == apModifierBase.MODIFIER_TYPE.AnimatedTF);
										}
									}
									break;

								case LAST_ANIM_TIMELINE_TYPE.ModMorph:
									{
										if(a._linkType == apAnimClip.LINK_TYPE.AnimatedModifier && a._linkedModifier != null)
										{
											return (a._linkedModifier.ModifierType == apModifierBase.MODIFIER_TYPE.AnimatedMorph);
										}
									}
									break;

								case LAST_ANIM_TIMELINE_TYPE.ModColor:
									{
										if(a._linkType == apAnimClip.LINK_TYPE.AnimatedModifier && a._linkedModifier != null)
										{
											return (a._linkedModifier.ModifierType == apModifierBase.MODIFIER_TYPE.AnimatedColorOnly);
										}
									}
									break;
							}
						...
						//이전에 선택한 것과 같은 타입의 적절한 타임라인을 찾았다.
						if(targetTimeline != null)
						{
							//Debug.Log("이전 타입의 타임라인을 선택하기 : " + _lastAnimTimelineType);
							SelectAnimTimeline(targetTimeline, true, true, false);
							isTimelineSelected = true;
						}
					}

					//이전의 타입과 같은 타입을 선택하지 않았다면, 그냥 첫번째 타임라인을 선택한다.
										...
										bool isAutoChangeLeftTab = true)
		{
			...
			//추가 [v1.4.5]
			//마지막으로 선택한 타임라인의 종류를 저장한다.
			if(_subAnimTimeline._linkType == apAnimClip.LINK_TYPE.ControlParam)
			{
				_lastAnimTimelineType = LAST_ANIM_TIMELINE_TYPE.ControlParam;
			}
			else if(_subAnimTimeline._linkType == apAnimClip.LINK_TYPE.AnimatedModifier)
			{
				if(_subAnimTimeline._linkedModifier != null)
				{
					switch (_subAnimTimeline._linkedModifier.ModifierType)
					{

						case apModifierBase.MODIFIER_TYPE.AnimatedTF:
							_lastAnimTimelineType = LAST_ANIM_TIMELINE_TYPE.ModTF;
							break;

						case apModifierBase.MODIFIER_TYPE.AnimatedMorph:
							_lastAnimTimelineType = LAST_ANIM_TIMELINE_TYPE.ModMorph;
							break;

						case apModifierBase.MODIFIER_TYPE.AnimatedColorOnly:
							_lastAnimTimelineType = LAST_ANIM_TIMELINE_TYPE.ModColor;
							break;
					}
				}
			}
