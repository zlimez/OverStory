...
namespace AnyPortrait
{
	...
	public static class apEditorUtil
	{
		...
		//private static Color s_delimeterColor_Pro = new Color(0.9f, 0.9f, 0.9f, 1.0f);
		//private static Color s_delimeterColor_Normal = new Color(0.7f, 0.7f, 0.7f, 1.0f);
		
		//[v1.4.6] 색상 변경 (Pro/Normal 구분 없이)
		private static Color s_delimeterColor = new Color(0.5f, 0.5f, 0.5f, 1.0f);

		public static void GUI_DelimeterBoxV(int height)
		{
			Color prevColor = GUI.backgroundColor;

			//if (EditorGUIUtility.isProSkin)	{ GUI.backgroundColor = s_delimeterColor_Pro; }
			//else							{ GUI.backgroundColor = s_delimeterColor_Normal; }
		...
		//ToggledButton_2Side_Sync 함수와 거의 같지만, Available이 false일 때도 클릭시 true를 리턴한다.
		public static bool ToggledButton_2Side_Sync_ForceClickEvent(	string strTextSelected,
																		string strTextNotSelected,
																		bool isEnabled, bool isAvailable, bool isSync,
																		int width, int height)
		{
			if (isEnabled || !isAvailable || !isSync)
			{
				...
				bool isBtn = GUILayout.Button(
					(isEnabled && isSync) ? strTextSelected : strTextNotSelected,
					(isAvailable == false ? apGUIStyleWrapper.I.Button_MiddleCenter_BoxPadding_White2Black : apGUIStyleWrapper.I.Button_MiddleCenter_BoxPadding_White2Cyan),
					apGUILOFactory.I.Width(width), apGUILOFactory.I.Height(height));
		...
		public static void ResetWhiteTexture()
		{
			if(_whiteSmallTexture != null)
			{
				...
				UnityEngine.Object.DestroyImmediate(_whiteSmallTexture);				
			}

			_whiteSmallTexture = null;

			//Debug.Log("White Texture를 생성한다.");

			//새로 생성하자
			Texture2D nativeWhiteTex = EditorGUIUtility.whiteTexture;

			if (nativeWhiteTex != null)
			{
				_whiteSmallTexture = UnityEngine.Object.Instantiate<Texture2D>(nativeWhiteTex);

				//Debug.Log("기본 Native Texture에서 복사함");
			}
			...
			if (_whiteSmallTexture == null)
			{
				...
				int nColors = _whiteSmallTexture.width * _whiteSmallTexture.height;
				Color[] whiteColors = new Color[nColors];

				for (int i = 0; i < nColors; i++)
				{
					whiteColors[i] = Color.white;
				}

				_whiteSmallTexture.SetPixels(whiteColors);
				...
				//	{
				//		_whiteSmallTexture.SetPixel(iX, iY, new Color(2.0f, 2.0f, 2.0f, 2.0f));
				//	}
				//}
			}
		...
		public static Texture2D WhiteTexture
		{
			get
			{
				//return EditorGUIUtility.whiteTexture;//<<이게 문제. 이게 Null일 수가 있다!

				//이거 다시 체크
				//버그가 있어서 다시 예전 방식을 사용한다. [v1.4.6]
				if (_whiteSmallTexture == null)
				{
					//가장 정확한 건 에디터의 WhiteTexture를 그대로 복사하는 것
					//만약 (유니티 에러로?) whiteTexture가 없다면, 직접 생성해야한다.
					Texture2D nativeWhiteTex = EditorGUIUtility.whiteTexture;
		...
		/// <summary>
		/// 반짝이는 UI를 만들 때 필요한 0~1의 값이 반복되는 애니메이션 가중치 값을 리턴한다.
		/// </summary>
		/// <returns></returns>
		public static float GetAnimatedHighlightWeight()
		{
			int milliSecond = System.DateTime.Now.Millisecond;
			return (Mathf.Sin(((float)milliSecond / 1000.0f) * Mathf.PI * 2.0f) * 0.5f) + 0.5f;
		...
		public static int GetAspectRatio_Height(int srcWidth, float aspectRatio)
		{
			...
			//return (int)(((float)srcWidth / aspectRatio) + 0.5f);
			return Mathf.RoundToInt((float)srcWidth / aspectRatio);
		}

		//public static int GetAspectRatio_Width(int srcHeight, int targetWidth, int targetHeight)
		public static int GetAspectRatio_Width(int srcHeight, float aspectRatio)
		{
			//float targetAspectRatio = (float)targetWidth / (float)targetHeight;
			//Aspect = W / H
			//W = H * Aspect <<
			...
			//return (int)(((float)srcHeight * aspectRatio) + 0.5f);
			return Mathf.RoundToInt((float)srcHeight * aspectRatio);
		}
		...
		/// 유니티 프로젝트를 저장한다.
		/// </summary>
		public static void SaveUnityProject()
		{
			//열려있는 모든 씬을 저장하고
			EditorSceneManager.SaveOpenScenes();

			//Save Project 메뉴를 실행한다. > 이건 패스
