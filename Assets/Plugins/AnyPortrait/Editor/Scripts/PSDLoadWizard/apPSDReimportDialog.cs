...
namespace AnyPortrait
{
	public partial class apPSDReimportDialog : EditorWindow
	{
		...
		private void DrawMeshGroup(apMeshGroup meshGroup)
		{
			...
			//	//<NoMod Pos를 계산한다.
			//	renderUnit.CalculateRenverVertsWorldPosNoMod(false);
			//}

			//변경 v1.4.4
			//Root Render Unit을 타겟으로 Child로 재귀적으로 호출하게 만들자
			if(meshGroup._rootRenderUnit != null)
			{
				meshGroup._rootRenderUnit.CalculateRenverVertsWorldPosNoMod(apRenderUnit.CALCULATE_NO_MOD_POS_REQUEST.RefreshWorldMatrix_WithChild);
			}
