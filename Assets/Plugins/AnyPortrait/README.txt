...
1.4.4 (March 29, 2023)
- The performance of the editor/in-game is slightly improved 
...
1.4.5 (July 15, 2023)
- The "Indexed" property to make animation not interpolate continuously is added
- The "Category" function is added to easily find animation event presets
...
- The "Reset" button is added to animation keyframe's properties UI
- The "Paste" function of the property UI of animation keyframes is improved so that users can select specific properties when targeting a mesh.
...
- "Save scenes after Bake" option is added to the Bake dialog
- The overloading of script functions that control the color, image or etc. of custom properties of mesh materials is configured to be similar to normal script functions.
- It is improved to reduce small vibrations of vertices when "Pulling by touch" is implemented using scripts.
- When "Root Unit" is selected in the editor, the order of animation clips on the right screen is changed to be the same as the order on the left of the editor.
...
1.4.6 (August 21, 2023)
- A new capture method (Version 2) is added that solves the problem of aspect ratio distortion and excessive blur caused by resizing.
...
- Options for the default color of the timeline UI and the color effect when selected are added to the settings dialog.
...
1.4.4 (2023년 3월 29일)
- 에디터/인게임의 성능이 조금 향상됨
- 선택된 컨트롤 파라미터를 타임라인에 등록하는 버튼이 비활성화되어 있는 문제가 수정됨
- 메시 그룹의 메시나 자식 메시 그룹의 크기나 회전값이 기본값이 아닌 경우에 Physics 모디파이어가 정상적으로 동작하지 않는 문제가 수정됨

1.4.5 (2023년 7월 15일)
- 애니메이션에서 연속적으로 보간하지않도록 만드는 "인덱스" 속성이 추가됨
- 애니메이션 이벤트 프리셋을 쉽게 찾을 수 있도록 "카테고리" 기능이 추가됨
- 애니메이션 이벤트의 프리셋이 알파벳 순서로 자동으로 정렬되도록 개선됨
- 애니메이션 이벤트의 프리셋을 이용하여 선택된 프레임에 바로 이벤트를 생성하는 버튼이 추가됨
- 편집 중인 애니메이션 클립이 변경되면, 애니메이션 이벤트 다이얼로그의 내용이 변경되도록 개선됨
- 애니메이션 키프레임의 속성 UI에 "Reset" 버튼이 추가됨
- 애니메이션 키프레임의 속성 UI의 "Paste" 기능이 메시를 대상으로 할 경우, 특정 속성을 선택할 수 있도록 개선됨
- 컨트롤 파라미터를 대상으로 하는 키프레임의 경우 UI 탭의 글자가 "Control Parameter"로 보이도록 변경됨
- 애니메이션 클립을 선택했을 때, 마지막으로 선택되었던 타임라인과 같은 종류의 타임라인이 가능한 선택되도록 변경됨
- 타임라인 레이어의 UI상의 기본 색상이 조금 더 밝고 선명하도록 변경됨
- 본을 대상으로 하는 타임라인 레이어의 UI상의 기본 색상이 본의 색상과 유사하도록 개선됨
- 선택된 타임라인이나 타임라인 레이어의 UI 색상에 약간의 점멸 효과를 넣어서 구별이 쉽도록 개선됨
- 이름 변경 단축키(기본 F2 키)를 누르면 "이름 변경 다이얼로그"가 나타나도록 변경됨 (기존에는 이름 속성 UI가 포커싱됨)
- Bake 다이얼로그에 "Bake를 하면 자동으로 씬이 저장되는 옵션"이 추가됨
- 메시 재질의 임의 속성의 색상이나 이미지를 제어하는 -"Custom"이 붙은-함수의 오버로딩 구성이 일반 함수와 유사하도록 개선됨
- 스크립트를 이용하여 "터치에 의한 당기기"를 구현했을 때, 버텍스들의 작은 진동들이 줄어들도록 개선됨
- 에디터에서 "루트 유닛"을 선택했을 때, 오른쪽 화면의 애니메이션 클립 순서가 에디터 왼쪽의 순서와 같아지도록 변경됨
- Bake에 의해서 생성된 GameObject를 임의로 삭제하고 다시 Bake를 할 경우 에러가 발생하여 Bake가 되지 않는 문제가 수정됨
- 기즈모나 타임라인 UI가 보여지는 상태에서 마우스 입력시 화면이 이상하게 보여지는 문제가 수정됨
- 스크립트로 애니메이션 재생시 알 수 없는 이유로 잘못된 데이터가 입력되어 에러가 발생하는 문제가 수정됨
- 단축키에 의해서 다이얼로그가 열리고 난 직후 단축키 입력이 1회 무시되는 문제가 수정됨
- Mac에서 일부 다이얼로그에서 버튼이 잘려보이는 문제가 수정됨
- 편집 대상이 바뀐 이후에도 컨트롤 파라미터나 모디파이어의 "일시적으로 동작 비활성하기"가 해제되지 않는 문제가 수정됨
- 메시의 부모 메시 그룹 객체의 Transform이 변경될 경우, 에디터 내에서의 버텍스 물리 연산과 Auto-Rig 기능이 비정상적으로 동작하는 문제가 수정됨
- 키프레임을 선택한 후, Shift키를 눌러서 여러개의 타임라인 레이어를 선택하려고 할 때 의도하지 않은 타임라인 레이어들이 선택되는 문제가 수정됨

1.4.6 (2023년 8월 21일)
- 화면 비율 왜곡 문제와 리사이징에 따른 블러 문제가 해결된 처리된 캡쳐 방식(Version 2)이 도입됨
- 설정 다이얼로그에서 기존의 캡쳐 방식(Version 1:Legacy)과 새로운 방식을 전환할 수 있음
- 결과 이미지 파일의 해상도 기준에서 픽셀 단위로 포커스를 미세 조정할 수 있는 옵션이 추가됨
- 캡쳐 프레임의 위치 옵션 중 Y 값의 부호가 기존과 반대로 적용되도록 변경됨
- 타임라인 UI의 기본 색상과 선택시 색상 효과에 대한 옵션이 설정 다이얼로그에 추가됨
- 타임라인 UI의 레이어의 글자 색상이 대체로 동일하게 보여지도록 개선됨
- 일부 데모 씬에서 게임 실행시 커서가 사라지는 스크립트가 유니티 에디터에서는 실행되지 않도록 변경됨
- 에디터의 구분 막대의 색상과 형태가 변경됨
- Extra Option 다이얼로그에서 Detph나 Image 옵션 버튼을 눌러서 활성화하면 자동으로 Extra Option 자체도 활성화되도록 개선되어 불필요한 클릭 횟수를 줄일 수 있음
- 에디터 내에서의 렌더링과 관련된 일부 코드가 개선됨
- 애니메이션 편집 화면에서 Extra Option에서 메시의 Depth가 변경될 때, 애니메이션 도중에 대상이 되는 메시들이 바뀌면 Depth 연산이 비정상적 수행되는 문제가 수정됨
