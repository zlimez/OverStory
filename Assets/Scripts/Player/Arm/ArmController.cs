...
public class ArmController : MonoBehaviour
{
    ...
    [Header("反撐")]
    [SerializeField][Tooltip("反撐時偵測玩家與他物碰撞的距離")] float playerColDetDist = 1.5f;
    ...
    [SerializeField][Tooltip("偵測拳頭與他物碰撞的距離")] float fistColDetDist = 1f;

    readonly Queue<(Trigger, object)> _triggerQueue = new();
    bool _isHard = false, _hasNewEnd = false, _fiRetBeg = false, _willAppImp = false;
    Vector2 _newEnd;
    ...
    void Move()
    {
        ...
        else fist.transform.position += speed * Time.fixedDeltaTime * (Vector3)Taim;
    }
    ...
    void OnSoCont(object input = null)
    {
        _fistExt.OnContact = null;
        _fistExt.OnCollision = null;
        _fistJoint.distance = rope.ropeLength;
    ...
    void Ext()
    {
        ...
        rope.linePoints = Math.Clamp(Mathf.CeilToInt(rope.ropeLength / segLength), minSegCount, maxSegCount);
        if (rope.ropeLength > maxLength - Utils.Const.EPS) QueueEvent(Trigger.Ret, true);
    }
    ...
    void OnHaRet(object isRet)
    {
        ...
        _hasNewEnd = true;
        _newEnd = rope.EndPoint.position + (rope.StartPoint.position - rope.EndPoint.position).normalized * rope.ropeLength;
    ...
    void Repel()
    {
        ...
        if (rope.ropeLength > maxLength - Utils.Const.EPS || Physics2D.Raycast(playerCtr.transform.position, repdir, playerColDetDist, Abyss.Settings.LayerMask.OBSTACLE_LMASK)) QueueEvent(Trigger.Ret, true);
    }
    ...
    void EntHaReLo(object input = null) => fist.transform.parent = transform;
    ...
    void AddEntryAction(State state, Action<object> action) => _fsm.AddEntryAction((int)state, action);
    void AddInAction(State state, Action action) => _fsm.AddInAction((int)state, action);
    void AddExitAction(State state, Action<object> action) => _fsm.AddExitAction((int)state, action);
    ...
    void AddStateActions()
    {
        ...
        AddInAction(State.So_CoEx_Lo, Ext);
        AddInAction(State.Ha_CoEx_Lo, HaExt);
    ...
    void AddTransitions()
    {
        ...
        AddTransition(State.So_CoEx_Lo, Trigger.Contact, State.So_CoRe_Lo, OnSoCont);
        ...
        AddTransition(State.So_CoRe_Fi, Trigger.ExtRel, State.So_CoRe_Lo, OnRel);
    ...
    void OnDrawGizmos()
    {
        ...
        Gizmos.DrawLine(playerCtr.transform.position, playerCtr.transform.position + (rope.EndPoint.position - rope.StartPoint.position).normalized * playerColDetDist);
    }
#endif
}
