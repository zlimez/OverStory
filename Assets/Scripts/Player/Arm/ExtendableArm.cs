...
namespace Abyss.Player
{
    ...
    public class ExtendableArm
    {
        ...
        private readonly Transform _fist, _disconnectedFist, _shoulder;
        ...
        public Vector2 CurrAim { get; private set; }
        ...
        private readonly float _playerCollisionDetectionRadius; // 反撐時偵測玩家與他物碰撞的距離
        private readonly ClampedFloatParameter _noGravRng; // 數值愈低玩家反撐且不受重力影響的角度愈廣

        private readonly float _disconnectedSpeed; // 脫離速度
        private readonly float _fistCollisionDetectionRadius; // 偵測拳頭與他物碰撞的距離
        ...
        private Vector2 _newEndPos;
        private readonly Rigidbody2D _fistRb;
        private readonly PolygonCollider2D _fistCollider;
        private readonly CollisionEventSource _fistCollisionEventSource;

        private Vector2 _lastFistPointPos, _nextFistPointPos, _lastShoulderPointPos, _nextShoulderPointPos;
        private float _timeSincePointPosUpdate;
        
        private float _computeDeltaTime;
        
        #region Getters and Setters
        public Vector2 UserAim { get; private set; } = Vector2.right;
        ...
        public bool HardExtended => CurrState is State.Ha_CoEx_Lo or State.Ha_CoRe_Lo;
        ...
        public Vector2 AnchorDir => Rope.Start.position - Rope.End.position;
        #endregion
        ...
        public ExtendableArm(ArmArgs armArgs)
        {
            _cam = armArgs.Cam;
            _fist = armArgs.Fist;
            _disconnectedFist = armArgs.DisconnectedFist;
            ...
            _playerCollisionDetectionRadius = armArgs.PlayerCollisionDetectionDistance;
            _fistCollisionDetectionRadius = armArgs.FistCollisionDetectionDistance;
            _disconnectedSpeed = armArgs.DisconnectedSpeed;
            _noGravRng = armArgs.NoGravityRangeAtRepel;
            
            _fistRb = _fist.GetComponent<Rigidbody2D>();
            _fistCollisionEventSource = _fist.GetComponent<CollisionEventSource>();
            _fistCollider = _disconnectedFist.GetComponent<PolygonCollider2D>();

            _fist.gameObject.SetActive(false);
            _fistPh.SetActive(true);
            _disconnectedFist.gameObject.SetActive(false);
        ...
        public void StartStep(float deltaTime)
        {
            ...
            if (_willAppImp)
            {
                _fistRb.AddForce(UserAim * _softExtImp, ForceMode2D.Impulse);
                _willAppImp = false;
            }
        ...
        public void Render(float deltaTime)
        {
            ...
            if (!Rope.StartPinned)
                _fist.transform.position = Vector2.Lerp(_lastFistPointPos, _nextFistPointPos, _timeSincePointPosUpdate / _computeDeltaTime);
            if (!Rope.EndPinned)
                _shoulder.transform.position = Vector2.Lerp(_lastShoulderPointPos, _nextShoulderPointPos, _timeSincePointPosUpdate / _computeDeltaTime);
            _timeSincePointPosUpdate += deltaTime;
        }

        public void CompleteStep()
        {
            Rope.CompleteStepIfBegan();

            _lastFistPointPos = _nextFistPointPos;
            _nextFistPointPos = Rope.StartPointPos;
            _lastShoulderPointPos = _nextShoulderPointPos;
            _nextShoulderPointPos = Rope.EndPointPos;
            _timeSincePointPosUpdate = 0;
        }
        ...
        private void OnDisc(object input = null)
        {
            ...
            Connected = false;
            Rope.Start = _fist;
            Rope.End = _disconnectedFist;
            _fistRb.isKinematic = true;
            CurrAim = UserAim;
        }

        private void InMovingFist(object deltaTime)
        {
            if (CheckFistCollision(CurrAim)) QueueEvent(Trigger.Contact);
            else _fist.transform.position += _disconnectedSpeed * (float)deltaTime * (Vector3)CurrAim;
        }

        private void OnHaDiRet(object args)
        {
            var (isRet, deltaTime) = ((bool, float))args;
            _disconnectedFist.gameObject.SetActive(true);
            if (isRet)
            {
            ...
            Rope.End.position = Rope.Start.position - _hardLen * (Vector3)CurrAim;
            ...
            for (var i = 0; i < 4; i++) colVtx[i] = new(colVtx[i].x / _disconnectedFist.transform.lossyScale.x, colVtx[i].y / _disconnectedFist.transform.lossyScale.y);
            _fistCollider.SetPath(0, colVtx);
        }
        ...
        private void OnPickedByDrone(object input = null)
        {
            _disconnectedFist.gameObject.SetActive(false);
            EventManager.InvokeEvent(PlayEvents.GetArm, _fist.transform);
        }
        private void OnFistReattached(object input = null) => _fist.gameObject.SetActive(false);
        ...
        private void OnSoEx(object input = null)
        {
            ...
            _willAppImp = true;
            _fistCollisionEventSource.OnContact = other =>
            {
                if (!other.gameObject.CompareTag(Tag.Hookable)) return;
                QueueEvent(Trigger.Hooked);
            };
            _fistCollisionEventSource.OnCollision = _ => QueueEvent(Trigger.Contact);
        }
        ...
        private void OnSoExStop(object input = null)
        {
            _fistRb.isKinematic = true;
            _fistRb.velocity = Vector3.zero;
        }

        private void InSoEx(object input = null)
        {
        ...
        private void InHaEx(object deltaTime)
        {
            ...
            _hasNewEnd = true;
            _newEndPos = Rope.End.position + (Vector3)CurrAim * _hardLen;
            _fistRb.MovePosition(_newEndPos);
            if (_hardLen > Rope.MaxLength - Const.EPS) QueueEvent(Trigger.Ret, true);
        }

        private void OnHaCont(object input = null)
        {
            VertRepelling = Vector2.Dot(Vector2.down, UserAim) > _noGravRng.value;
        ...
        private void OnHaRet(object args)
        {
            ...
            _hasNewEnd = true;
            _newEndPos = Rope.End.position + (Rope.Start.position - Rope.End.position).normalized * _hardLen;
            _fistRb.MovePosition(_newEndPos);
        ...
        private void InRepel(object input = null)
        {
            _hardLen = (Rope.Start.position - Rope.End.position).magnitude;
            Vector2 repDir = (Rope.End.position - Rope.Start.position).normalized;
            if (_hardLen > Rope.MaxLength - Const.EPS || Physics2D.Raycast(_playerCtr.transform.position, repDir, _playerCollisionDetectionRadius, Settings.LayerMask.OBSTACLE_LMASK)) QueueEvent(Trigger.Ret, true);
        }
        ...
        {
            var cAim = (_hasNewEnd ? _newEndPos : _fistRb.transform.position) - Rope.End.position;
            _hasNewEnd = false;
            Vector3 naim = Quaternion.Euler(0, 0, Vector2.SignedAngle(cAim, UserAim) * _aimResp * (float)deltaTime) * cAim, nnaim = naim.normalized;
            if (Physics2D.BoxCast(Rope.End.position, new(0.5f, Rope.Width), Vector2.SignedAngle(Vector2.right, naim), nnaim, naim.magnitude, Settings.LayerMask.OBSTACLE_LMASK)) return;
            CurrAim = nnaim;
            _fistRb.MovePosition(Rope.End.position + naim);
        }

        private void EnterCoReLo(object input = null) => Rope.QueuePin(Rope.PinPoint.End);
        private void EnterHaReLo(object input = null) => _fist.transform.parent = _shoulder;
        ...
        private void SetAim() => UserAim = ((Vector2)(_cam.ScreenToWorldPoint(Input.mousePosition) - Rope.End.position)).normalized;
        private bool CheckFistCollision(Vector2 dir) => Physics2D.Raycast(_fist.transform.position, dir.normalized, _fistCollisionDetectionRadius, Abyss.Settings.LayerMask.OBSTACLE_LMASK);
        ...
        {
            AddInAction(State.Ha_DiMo, InMovingFist);
            AddInAction(State.So_DiMo, InMovingFist);
            AddInAction(State.So_CoEx_Lo, InSoEx);
            AddInAction(State.Ha_CoEx_Lo, InHaEx);
            AddInAction(State.Ha_CoEx_To, InRepel);

            AddEntryAction(State.So_CoRe_Lo, EnterCoReLo);
            AddEntryAction(State.Ha_CoRe_Lo, EnterHaReLo);
        ...
        private void AddTransitions()
        {
            ...
            AddTransition(State.So_CoEx_Lo, Trigger.Ret, State.So_CoRe_Lo, OnSoExStop);
            AddTransition(State.So_CoEx_Lo, Trigger.Hooked, State.So_CoRe_Fi, OnSoHook);
            AddTransition(State.So_CoEx_Lo, Trigger.Contact, State.So_CoRe_Lo, OnSoExStop);
            ...
            AddTransition(State.Ha_DiOg, Trigger.ExtRel, State.Ha_DiCa, OnPickedByDrone);
            AddTransition(State.So_DiOg, Trigger.ExtRel, State.So_DiCa, OnPickedByDrone);

            AddTransition(State.Ha_DiCa, Trigger.Kept, State.Ha_CoOg_Lo, OnFistReattached);
            AddTransition(State.So_DiCa, Trigger.Kept, State.So_CoOg_Lo, OnFistReattached);
